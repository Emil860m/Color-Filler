...
namespace CreativeSpore.SuperTilemapEditor
{
    public class MapTileNode : IPathNode
    {
        ...
        int[]
            m_neighborIdxList =
                new int[8]; // the index is a concat of the grid position (two int32) into a single int32
                    ...
                    Tile tile = tilemap.GetTile(
                        tilemap.transform
                            .InverseTransformPoint(
                                Position)); // Use Position instead to allow tilemaps with offset different than 0, tilemap.GetTile(GridX, GridY);
                    isEmptyCell = false;
        ...
        public override float GetNeigborMovingCost(int neigborIdx)
        {
            ...
            if (neigborIdx == 0 || neigborIdx == 2 || neigborIdx == 5 || neigborIdx == 7)
            {
                ...
                bool usingColliders = (m_owner.PassableDetectionMode &
                ...
                bool nodeNisPassable = nodeN.IsPassable() &&
                                       (!usingColliders || GetNeigborMovingCost(1) != PathFinding.k_InfiniteCostValue);
                bool nodeWisPassable = nodeW.IsPassable() &&
                                       (!usingColliders || GetNeigborMovingCost(3) != PathFinding.k_InfiniteCostValue);
                bool nodeEisPassable = nodeE.IsPassable() &&
                                       (!usingColliders || GetNeigborMovingCost(4) != PathFinding.k_InfiniteCostValue);
                bool nodeSisPassable = nodeS.IsPassable() &&
                                       (!usingColliders || GetNeigborMovingCost(6) != PathFinding.k_InfiniteCostValue);
                if (
        ...
        public override int GetNeighborCount()
        {
            return 8;
        }
        ...
        {
            return m_owner.GetMapTileNode(m_neighborIdxList[neighbourIdx]);
        }
    ...
    public class MapPathFinding
    {
        ...
        {
            get { return m_pathFinding.IsComputing; }
        }
