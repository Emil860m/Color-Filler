...
namespace CreativeSpore.SuperTilemapEditor.PathFindingLib
{
    public class IPathNode
    {
        ...
        public virtual int GetNeighborCount()
        {
            return 0;
        }

        public virtual IPathNode GetNeighbor(int nodeIdx)
        {
            return null;
        }

        public virtual float GetNeigborMovingCost(int neigborIdx)
        {
            return 1f;
        }

        public virtual float GetHeuristic()
        {
            return 0f;
        } //H
        ...
        public IEnumerator ComputePathCoroutine(FindingParams findingParams)
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    curNode = null;
                    for (LinkedListNode<IPathNode> pathNode = m_openList.First;
                        pathNode != null;
                    ...
                    curNode.Value.openComputeId = 0;
                    m_closeList.AddLast(curNode);
                    curNode.Value.closeComputeId = m_computeId;
                ...
                if (iterations >= MaxIterations)
                    Debug.LogWarning(
                        "Info: max iterations reached before finding path solution. MaxIterations is set to " +
                        MaxIterations);
                //d) Stop when you:
