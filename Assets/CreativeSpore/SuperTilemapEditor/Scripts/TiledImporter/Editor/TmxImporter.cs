    ...
    public static class TmxImporter
    {
        ...
        public static Tileset CreateTilesetFromTmx(string tmxFilePath, string dstPath)
        {
            ...
            else
            {
                ...
                tilesetRects =
                    atlasTexture.PackTextures(tilemap.DicTilesetTex2D.Values.Select(x => x.tilesetTexture).ToArray(), 0,
                        8192);
            }

            string atlasPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath(tilesetAsset)) + "/" + tmxFileName +
                               "Atlas.png";
            ...
            tilesetAsset.TilePxSize =
                new Vector2(tilemap.Map.Tilesets[0].TileWidth, tilemap.Map.Tilesets[0].TileHeight);
            int tilesetIdx = 0;
            ...
            foreach (TmxTilemap.TilesetTextureData tilesetTextureData in tilemap.DicTilesetTex2D.Values)
            {
                ...
                Vector2 tileSize = new Vector2(tilemap.Map.Tilesets[tilesetIdx].TileWidth,
                    tilemap.Map.Tilesets[tilesetIdx].TileHeight);
                Rect[] tileRects = tilesetTextureData.isCollectionOfSprites
                    ? tilesetTextureData.tileRects
                    : GenerateGridSpriteRectangles(tilesetTextureData.tilesetTexture,
                        new Vector2(tmxTileset.Margin, tmxTileset.Margin), tileSize,
                        new Vector2(tmxTileset.Spacing, tmxTileset.Spacing));
                TileSelection tilesetSelection = tilesetTextureData.isCollectionOfSprites
                    ...
                    : new TileSelection(
                        Enumerable.Range(tileList.Count, tileRects.Length).Select(i => (uint) i).ToList(),
                        tmxTileset.Columns);
                for (int i = 0; i < tileRects.Length; ++i)
                {
                    ...
                    {
                        int paddingTiles = i == 0
                            ? tilesetTextureData.tileIds[i]
                            : (tilesetTextureData.tileIds[i] - tilesetTextureData.tileIds[i - 1]) - 1;
                        if (paddingTiles > 0)
                    ...
                    if (tileData != Tileset.k_TileData_Empty)
                    {
                        ...
                        if ((tileData & Tileset.k_TileFlag_Rot90) != 0)
                        {
                            if (((tile.GId & k_FLIPPED_HORIZONTALLY_FLAG) != 0) !=
                                ((tile.GId & k_FLIPPED_VERTICALLY_FLAG) != 0))
                            {
        ...
        public static Rect[] GenerateGridSpriteRectangles(Texture2D texture, Vector2 offset, Vector2 size,
            Vector2 padding)
        {
