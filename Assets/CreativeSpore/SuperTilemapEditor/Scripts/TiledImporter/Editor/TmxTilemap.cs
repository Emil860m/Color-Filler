...
namespace CreativeSpore.TiledImporter
{
    public class TmxTilemap
    {
        ...
        public Dictionary<int, TilesetTextureData> DicTilesetTex2D
        {
            get { return m_dicTilesetTex2d; }
        }
        ...
        public class TilesetTextureData
        {
            ...
            public bool isCollectionOfSprites
            {
                get { return tileRects != null && tileRects.Length > 0; }
            }
        }
        ...
        private void LoadImageData()
        {
            foreach (TmxTileset tileset in m_map.Tilesets)
            {
                    ...
                    if (tileset.TilesWithProperties.Count > 0)
                    {
                        ...
                        foreach (TmxTile tmxTile in tileset.TilesWithProperties)
                        {
                            if (tmxTile.Image != null)
                                tileImages.Add(LoadTexture(tmxTile.Image.Width, tmxTile.Image.Height,
                                    tmxTile.Image.Source, tmxTile.Id.ToString()));
                        }
                        ...
                        m_dicTilesetTex2d.Add(
                            tileset.FirstGId,
                            new TilesetTextureData()
                            {
                                tilesetTexture = atlasTexture,
                                tileRects = tileRects,
                                tileIds = tileset.TilesWithProperties.Select(x => x.Id).ToArray()
                            }
                        );
                    }
                    else
                ...
                {
                    Texture2D texture = LoadTexture(tileset.Image.Width, tileset.Image.Height, tileset.Image.Source,
                        tileset.Name);
                    m_dicTilesetTex2d.Add(
                        tileset.FirstGId,
        ...
        void ImportIntoScene(TmxLayer layer)
        {
            ...
            for (int tile_y = 0; tile_y < layer.Height; tile_y++)
            {
                ...
                Rect dstRect = new Rect(tile_x * objTileset.TileWidth,
                    (layer.Height - 1 - tile_y) * objTileset.TileHeight, objTileset.TileWidth, objTileset.TileHeight);
                ...
                Rect srcRect = new Rect(tileset_x * objTileset.TileWidth,
                    texTileset.height - (tileset_y + 1) * objTileset.TileHeight, objTileset.TileWidth,
                    objTileset.TileHeight);
