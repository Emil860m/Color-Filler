...
namespace CreativeSpore.TiledImporter
{
    ...
    public class TmxMap
    {
        ...
        [XmlAttribute("tilewidth")] public string TileWidth { get; set; }

        [XmlAttribute("tileheight")] public string TileHeight { get; set; }
        ...
        internal void FixExportedTilesets(string relativePath)
        {
            ...
            for (int i = 0; i < Tilesets.Count; ++i)
            {
                ...
                if (!string.IsNullOrEmpty(tmxTileset.Source))
                {
                    ...
                    Tilesets[i] = tmxTileset =
                        objSerializer.LoadFromXMLFile<TmxTileset>(Path.Combine(relativePath, tmxTileset.Source));
                    tmxTileset.FirstGId = firstGid;
                    if (tmxTileset.TileCount == 0)
                    {
                        int horTiles = System.Convert.ToInt32(Math.Round(
                            (float) (tmxTileset.Image.Width - 2 * tmxTileset.Margin) /
                            (tmxTileset.TileWidth + tmxTileset.Spacing)));
                        int verTiles = System.Convert.ToInt32(Math.Round(
                            (float) (tmxTileset.Image.Height - 2 * tmxTileset.Margin) /
                            (tmxTileset.TileHeight + tmxTileset.Spacing)));
