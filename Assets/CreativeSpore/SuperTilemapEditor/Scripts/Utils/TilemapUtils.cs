...
namespace CreativeSpore.SuperTilemapEditor
{
    public class TileData
    {
        ...
        public bool IsEmpty
        {
            get { return brushId == Tileset.k_BrushId_Default && tileId == Tileset.k_TileId_Empty; }
        }
        ...
        public void SetData(uint tileData)
        {
            ...
            brushId = tileData != Tileset.k_TileData_Empty
                ? (int) ((tileData & Tileset.k_TileDataMask_BrushId) >> 16)
                : 0;
    ...
    public static class TilemapUtils
    {
        public static Material FindDefaultSpriteMaterial()
        {
            ...
            return
                UnityEditor.AssetDatabase
                    .GetBuiltinExtraResource<Material
                    >("Sprites-Default.mat"); //fix: Unity 5.4.0f3 is not finding the material using Resources
#else
        ...
        {
            return tilemap.transform.TransformPoint(new Vector2((gridX + .5f) * tilemap.CellSize.x,
                (gridY + .5f) * tilemap.CellSize.y));
        }
        ...
        /// <param name="tilemap"></param>
        /// <param name="action"></param>
        static public void IterateTilemapWithAction(STETilemap tilemap,
            System.Action<STETilemap, int, int, uint> action)
        {
            if (tilemap)
                for (int gy = tilemap.MinGridY; gy <= tilemap.MaxGridY; ++gy)
                for (int gx = tilemap.MinGridX; gx <= tilemap.MaxGridX; ++gx)
        ...
        public static Camera GetCurrentPreviewCamera()
        {
        ...
        public static Texture2D CreateTilemapGroupPreviewTexture(STETilemap tilemap, TilemapGroup tilemapGroup = null)
        {
            ...
            if (tilemapGroup)
                tilemapGroup.IterateTilemapWithAction((STETilemap tmap) =>
                    tilemapGroupBounds.Encapsulate(tmap.MapBounds));
            ...
            previewCamera.transform.position =
                tilemap.transform.TransformPoint(new Vector3(tilemapGroupBounds.center.x, tilemapGroupBounds.center.y,
                    -10));
