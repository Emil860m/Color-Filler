...
namespace CreativeSpore.SuperTilemapEditor
{
    public static class TilemapUtilsEditor
    {
        ...
        static public Texture2D CreateTexture2DFromTilemap(STETilemap tilemap)
        {
            ...
            Texture2D output = new Texture2D(tilemap.GridWidth * tilePxSizeX, tilemap.GridHeight * tilePxSizeY,
                TextureFormat.ARGB32, false);
            output.filterMode = FilterMode.Point;
            output.SetPixels32(new Color32[output.width * output.height]);
            output.Apply();
            System.Action<STETilemap, int, int, uint> action =
                (STETilemap source, int gridX, int gridY, uint tileData) =>
                {
                    ...
                    if (tile != null)
                    {
                        ...
                        if (prefabSprite)
                        {
                            ...
                            Vector2
                                offset = pivot - tilemap.Tileset.TilePxSize /
                                         2; // sprite.pivot + sprite.textureRect.position - sprite.textureRectOffset;            
                            BlitPixels(output, gridX * tilePxSizeX - Mathf.RoundToInt(offset.x),
                                gridY * tilePxSizeY - Mathf.RoundToInt(offset.y), Mathf.RoundToInt(tileSize.x),
                                Mathf.RoundToInt(tileSize.y), srcTileColors);
                        }
                        else
                        {
        ...
        public static void BlitPixels(Texture2D texture, int x, int y, int blockWidth, int blockHeight,
            Color[] blitColors)
        {
            Debug.Assert(x >= 0 && y >= 0 && (x + blockWidth) <= texture.width && (y + blockHeight) <= texture.height,
                "BlitPixels destination rectangle is out of bounds!");
