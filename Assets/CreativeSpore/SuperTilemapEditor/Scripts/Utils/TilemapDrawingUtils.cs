...
namespace CreativeSpore.SuperTilemapEditor
{
    public static class TilemapDrawingUtils
    {
                ...
                while (check.Count > 0)
                {
                    ...
                    })
                    {
                        ...
                        )
                        {
                            if (
                                isBrush
                                    ? Tileset.GetBrushIdFromTileData(floodFrom) ==
                                      Tileset.GetBrushIdFromTileData(nextTileData)
                                    : floodFrom == nextTileData
                            )
                            {
                                check.AddLast(next);
                                dataIdx0 = randomize
                                    ? Random.Range(0, patternW)
                                    : (next.X % patternW + patternW) % patternW;
                                dataIdx1 = randomize
                                    ? Random.Range(0, patternH)
                                    : (next.Y % patternH + patternH) % patternH;
                    ...
                    {
#if UNITY_EDITOR
                        int result = UnityEditor.EditorUtility.DisplayDialogComplex("FloodFill is taking too much time",
                            "Do you want to continue for another " + k_timeToAbortFloodFill + " seconds?", "Wait",
                            "Cancel", "Wait and Don't ask again");
                        if (result == 0)
                        {
                            timeStamp = Time.realtimeSinceStartup;
                        }
        ...
        public static void FloodFillPreview(STETilemap tilemap, Vector2 vLocalPos, uint tileData,
            List<Vector2> outFilledPoints, uint maxPoints = uint.MaxValue)
        {
            int gridX = BrushUtil.GetGridX(vLocalPos, tilemap.CellSize);
            int gridY = BrushUtil.GetGridY(vLocalPos, tilemap.CellSize);
        ...
        public static void FloodFillPreview(STETilemap tilemap, int gridX, int gridY, uint tileData,
            List<Vector2> outFilledPoints, uint maxPoints = uint.MaxValue)
        {
            if (
                ...
                )
                {
                    ...
                    while (check.Count > 0)
                    {
                        ...
                        })
                        {
                            ...
                            {
                                if (filledPoints[
                                    (next.Y - tilemap.MinGridY) * tilemap.GridWidth + next.X - tilemap.MinGridX])
                                ...
                                )
                                {
                                    check.AddLast(next);
                                    filledPoints[
                                            (next.Y - tilemap.MinGridY) * tilemap.GridWidth + next.X -
                                            tilemap.MinGridX] =
                                        true;
        ...
        public static void DrawLine(STETilemap tilemap, Vector2 locPosA, Vector2 locPosB, uint[,] tileData,
        ...
        public static void DrawLineMirrored(STETilemap tilemap, int x0, int y0, int x1, int y1, uint[,] tileData,
            bool randomize = false)
        {
            int w = tileData.GetLength(0);
            int h = tileData.GetLength(1);
        ...
        public static void DrawRect(STETilemap tilemap, Vector2 locPosA, Vector2 locPosB, uint[,] tileData,
            bool isFilled, bool is9Sliced = false, bool randomize = false)
        {
            int x0 = TilemapUtils.GetGridX(tilemap, locPosA);
            int y0 = TilemapUtils.GetGridY(tilemap, locPosA);
            int x1 = TilemapUtils.GetGridX(tilemap, locPosB);
        ...
        public static void DrawRect(STETilemap tilemap, int x0, int y0, int x1, int y1, uint[,] tileData, bool isFilled,
            bool is9Sliced = false, bool randomize = false)
        {
            int w = tileData.GetLength(0);
            int h = tileData.GetLength(1);
            if (x0 > x1) Swap<int>(ref x0, ref x1);
                ...
                (x, y) =>
                {
                    if (is9Sliced)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (randomize)
                                    tilemap.SetTileData(x, y,
                                        tileData[w > 2 ? Random.Range(1, w - 1) : cx,
                                            h > 2 ? Random.Range(1, h - 1) : cy]);
                                else
        ...
        public static void Ellipse(int x1, int y1, int x2, int y2, bool isFilled, PlotFunction plot)
        {
            ...
            if (isFilled)
            {
                ...
                if (rx == 1)
                {
                    for (int c = ymin; c <= ymax; ++c) plot(x2, c);
                    rx--;
                }

                if (rx == 0)
                {
                    for (int c = ymin; c <= ymax; ++c) plot(x1, c);
                    return;
                }

                if (ry == 1)
                {
                    for (int c = xmin; c <= xmax; ++c) plot(c, y2);
                    ry--;
                }

                if (ry == 0)
                {
                    for (int c = xmin; c <= xmax; ++c) plot(c, y1);
                    return;
                }
            }
            else
            {
                if (rx == 1)
                {
                    Line(x2, y1, x2, y2, plot);
                    rx--;
                }

                if (rx == 0)
                {
                    Line(x1, y1, x1, y2, plot);
                    return;
                }

                if (ry == 1)
                {
                    Line(x1, y2, x2, y2, plot);
                    ry--;
                }

                if (ry == 0)
                {
                    Line(x1, y1, x2, y1, plot);
                    return;
                }
            }
        ...
        public static void DrawEllipse(STETilemap tilemap, Vector2 locPosA, Vector2 locPosB, uint[,] tileData,
            bool isFilled, bool randomize = false)
        {
            int x0 = TilemapUtils.GetGridX(tilemap, locPosA);
            int y0 = TilemapUtils.GetGridY(tilemap, locPosA);
            int x1 = TilemapUtils.GetGridX(tilemap, locPosB);
