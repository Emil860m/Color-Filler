    ...
    public static class GizmosEx
    {
        public static float GetGizmoSize(Vector3 position)
        {
            ...
            if (current)
            {
                ...
                Vector3 a = current.WorldToScreenPoint(
                    position2 + transform.TransformDirection(new Vector3(0f, 0f, z)));
                Vector3 b = current.WorldToScreenPoint(
                    position2 + transform.TransformDirection(new Vector3(1f, 0f, z)));
        ...
        {
            Rect rDot = new Rect(-size / (2 * transform.localScale.x), -size / (2 * transform.localScale.y),
                size / transform.localScale.x, size / transform.localScale.y);
            Vector3[] rectVerts =
            {
