...
namespace CreativeSpore.SuperTilemapEditor
{
    public static class TilemapVertexPaintUtils
    {
        ...
        public static void VertexPaintCircle(STETilemap tilemap, Vector2 center, float radius, Color color,
            eBlendMode blendingMode, bool vertexPaint = false, AnimationCurve intensityCurve = null)
        {
            ...
            for (int y = minGridY; y <= maxGridY; ++y, tileCenter.y += tilemap.CellSize.y)
            {
                ...
                for (int x = minGridX; x <= maxGridX; ++x, tileCenter.x += tilemap.CellSize.x)
                {
                    if (vertexPaint)
                    {
                        _PaintTileVerticesByDist(tilemap, x, y, tileCenter, center, radius, color, blendingMode,
                            intensityCurve);
                    }
                    else
                    {
        ...
        private static void _PaintTileVerticesByDist(STETilemap tilemap, int gridX, int gridY, Vector2 tilePos,
            Vector2 center, float radius, Color color, eBlendMode blendingMode, AnimationCurve intensityCurve)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                if (sqrDist <= sqrRadius)
                {
                    ...
                    {
                        float dist = Mathf.Sqrt(sqrDist);
                        s_tileVertexColor[i] = new Color32(color32.r, color32.g, color32.b,
                            (byte) (255 * color.a * intensityCurve.Evaluate(1f - dist / radius)));
                    }
                    else
                    {
            ...
            tilemap.SetTileColor(gridX, gridY,
                new TileColor32(s_tileVertexColor[0], s_tileVertexColor[1], s_tileVertexColor[2], s_tileVertexColor[3]),
                blendingMode);
        }
    }
}