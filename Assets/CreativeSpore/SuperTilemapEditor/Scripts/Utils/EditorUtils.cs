...
namespace CreativeSpore.SuperTilemapEditor
{
    public static class EditorUtils
    {
        public static T CreateAssetInSelectedDirectory<T>(string name = null) where T : ScriptableObject
        {
            ...
            ProjectWindowUtil.CreateAsset(asset,
                (string.IsNullOrEmpty(name) ? "New " + typeof(T).Name : name) + ".asset");
            return asset;
            /*
        ...
        {
            System.Type internalEditorUtilityType = typeof(InternalEditorUtility);
            PropertyInfo sortingLayersProperty = internalEditorUtilityType.GetProperty("sortingLayerUniqueIDs",
                BindingFlags.Static | BindingFlags.NonPublic);
        ...
        public static void SortingLayerFieldLayout(GUIContent label, SerializedProperty layerID, GUIStyle style,
            GUIStyle labelStyle)
        {
            Rect position = EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight, EditorStyles.popup,
            ...
            GUIStyle style, GUIStyle labelStyle)
        {
            ...
            EditorUtility.SelectMenuItemFunction setEnumValueDelegate =
                (object userData, string[] options, int selected) =>
                {
                    if (selected == options.Length - 1)
                    {
                        ...
                        PropertyInfo tagManagerPropertyInfo = typeof(EditorApplication).GetProperty("tagManager",
                            BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.GetProperty);
                        if (tagManagerPropertyInfo != null)
                        {
                            System.Object tagManager =
                                (System.Object) tagManagerPropertyInfo.GetValue(typeof(EditorApplication), null);
        ...
        public static bool DoToggleButton(string label, bool value)
        {
            ...
            if (GUILayout.Button(label,
                value
                    ? new GUIStyle("Button") {normal = {background = btnStyle.active.background}}
                    : (GUIStyle) "Button"))
            {
                value = !value;
            }
