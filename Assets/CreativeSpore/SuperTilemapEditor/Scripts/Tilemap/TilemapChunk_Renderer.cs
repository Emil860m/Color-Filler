...
namespace CreativeSpore.SuperTilemapEditor
{
    partial class TilemapChunk
    {
        ...
        public void SetTileColor(int locGridX, int locGridY, TileColor32 tileColor,
            eBlendMode blendMode = eBlendMode.AlphaBlending)
        {
            if (locGridX >= 0 && locGridX < m_width && locGridY >= 0 && locGridY < m_height)
            {
                ...
                {
                    m_tileColorList = Enumerable.Repeat(new TileColor32(new Color32(0xff, 0xff, 0xff, 0xff)),
                        m_tileDataList.Count).ToList();
                }
        ...
        public bool UpdateMesh()
        {
            ...
            if (EditorCompatibilityUtils.IsPrefab(gameObject))
            {
                ...
                if (m_meshRenderer.sharedMaterial == null || m_meshRenderer.sharedMaterial == ParentTilemap.Material)
                {
                    ...
                    m_meshRenderer.sharedMaterial.mainTexture =
                        ParentTilemap.Tileset ? ParentTilemap.Tileset.AtlasTexture : null;
                }
            }
            else
#endif
        ...
        public static void RegisterAnimatedBrush(IBrush brush, int subTileIdx = -1)
        {
            ...
            {
                if (subTileIdx >= 0)
                    s_currUpdatedTilechunk.m_animatedTiles.Add(new AnimTileData()
                    {
                        VertexIdx = s_currUVVertex + (subTileIdx << 2),
                        Brush = brush,
        ...
        */
            ...
            for (int ty = 0, tileIdx = 0; ty < m_height; ++ty)
            {
                for (int tx = 0; tx < m_width; ++tx, ++tileIdx)
                {
                    ...
                    if (tileData != Tileset.k_TileData_Empty)
                    {
                        ...
                        {
                            Debug.LogWarning(ParentTilemap.name + "\\" + name + ": TileId " + tileId +
                                             " not found! GridPos(" + (GridPosX + tx) + "," + (GridPosY + ty) +
                                             ") tilaData 0x" + tileData.ToString("X"));
                        ...
                        if (brushId > 0)
                        {
                            ...
                            if (tileBrush == null)
                            {
                                Debug.LogWarning(ParentTilemap.name + "\\" + name + ": BrushId " + brushId +
                                                 " not found! GridPos(" + (GridPosX + tx) + "," + (GridPosY + ty) +
                                                 ") tilaData 0x" + tileData.ToString("X"));
                            ...
                            if (tileBrush != null &&
                                (m_invalidateBrushes || (tileData & Tileset.k_TileFlag_Updated) == 0))
                            {
                                ...
                                if (BrushBehaviour.Instance.BrushTilemap == ParentTilemap
                                ) // avoid changing brushId when updating the BrushTilemap
                                {
                        ...
                        Rect tileUV;
                        uint[] subtileData = tileBrush != null
                            ? tileBrush.GetSubtiles(ParentTilemap, GridPosX + tx, GridPosY + ty, tileData)
                            : null;
                        if (subtileData == null)
                        {
                            if (tile != null)
                            {
                                if (tile.prefabData.prefab == null || tile.prefabData
                                                                       .showTileWithPrefab //hide the tiles with prefabs ( unless showTileWithPrefab is true )
                                                                   || tileBrush && tileBrush.IsAnimated()
                                ) // ( skip if it's an animated brush )
                                {
                                    tileUV = tile.uv;
                                    ...
                                    if (m_tileColorList != null && m_tileColorList.Count > tileIdx)
                                    {
                                        ...
                                        Color32 middleColor = new Color32(
                                            System.Convert.ToByte(
                                                (tileColor32.c0.r + tileColor32.c1.r + tileColor32.c2.r +
                                                 tileColor32.c3.r) >> 2),
                                            System.Convert.ToByte(
                                                (tileColor32.c0.g + tileColor32.c1.g + tileColor32.c2.g +
                                                 tileColor32.c3.g) >> 2),
                                            System.Convert.ToByte(
                                                (tileColor32.c0.b + tileColor32.c1.b + tileColor32.c2.b +
                                                 tileColor32.c3.b) >> 2),
                                            System.Convert.ToByte(
                                                (tileColor32.c0.a + tileColor32.c1.a + tileColor32.c2.a +
                                                 tileColor32.c3.a) >> 2)
                                        );
        ...
        private void _AddTileToMesh(Rect tileUV, int tx, int ty, uint tileData, Vector2 subtileOffset,
            Vector2 subtileCellSize, int subTileIdx = -1)
        {
