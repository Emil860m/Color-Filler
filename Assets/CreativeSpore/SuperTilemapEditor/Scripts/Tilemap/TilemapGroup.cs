...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilemapGroup : MonoBehaviour, ISerializationCallbackReceiver
    {
        public STETilemap SelectedTilemap
        {
            get
            {
                return m_selectedIndex >= 0 && m_selectedIndex < m_tilemaps.Count ? m_tilemaps[m_selectedIndex] : null;
            }
            set { m_selectedIndex = m_tilemaps != null ? m_tilemaps.IndexOf(value) : -1; }
        }

        public List<STETilemap> Tilemaps
        {
            get { return m_tilemaps; }
        }

        public float UnselectedColorMultiplier
        {
            get { return m_unselectedColorMultiplier; }
            set { m_unselectedColorMultiplier = value; }
        }

        public bool DefaultTilemapWindowVisible
        {
            get { return m_defaultTilemapWindowVisible; }
            set { m_defaultTilemapWindowVisible = value; }
        }

        public bool DisplayTilemapRList
        {
            get { return m_displayTilemapRList; }
            set { m_displayTilemapRList = value; }
        }

        public STETilemap this[int idx]
        {
            get { return m_tilemaps[idx]; }
        }

        public STETilemap this[string name]
        {
            get { return FindTilemapByName(name); }
        }

        [SerializeField] private List<STETilemap> m_tilemaps = new List<STETilemap>();
        [SerializeField] private int m_selectedIndex = -1;
        [SerializeField, Range(0f, 1f)] private float m_unselectedColorMultiplier = 1f;
        [SerializeField] private bool m_defaultTilemapWindowVisible = false;
        [SerializeField] private bool m_displayTilemapRList = true;
        ...
        [SerializeField] private List<int> m_savedTileIds = new List<int>();
        [SerializeField] private List<STETilemap> m_savedTileDefaultTilemaps = new List<STETilemap>();
        [SerializeField] private List<int> m_savedBrushIds = new List<int>();
        [SerializeField] private List<STETilemap> m_savedBrushDefaultTilemaps = new List<STETilemap>();
        ...
        public STETilemap GetDefaultTilemapForCurrentTileSelection()
        {
            ...
            {
                if (SelectedTilemap.Tileset.SelectedTileId != Tileset.k_TileId_Empty)
                    return GetDefaultTilemapForTileOrBrush(SelectedTilemap.Tileset.SelectedTileId,
                        m_dicTileDefaultTilemap);
                else if (SelectedTilemap.Tileset.SelectedBrushId != Tileset.k_BrushId_Default)
                    return GetDefaultTilemapForTileOrBrush(SelectedTilemap.Tileset.SelectedBrushId,
                        m_dicBrushDefaultTilemap);
                else if (SelectedTilemap.Tileset.TileSelection != null &&
                         SelectedTilemap.Tileset.TileSelection.selectionData.Count > 0)
                {
        ...
        public void ClearDefaultTilemapForCurrentSelectedTileOrBrush()
        {
            if (SelectedTilemap && SelectedTilemap.Tileset)
            {
                if (SelectedTilemap.Tileset.SelectedTileId != Tileset.k_TileId_Empty)
                    ...
                    ClearDefaultTilemapForTileOrBrush(SelectedTilemap.Tileset.SelectedBrushId,
                        m_dicBrushDefaultTilemap);
            }
        }
