...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public struct TilePrefabData
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            return (other.prefab == this.prefab) && (other.offset == this.offset) &&
                   (other.offsetMode == this.offsetMode);
        }
    ...
    {
        public List<uint> selectionData
        {
        ...
        {
            get { return 1 + (m_tileIds.Count - 1) / m_rowLength; }
        }

        [SerializeField] private int m_rowLength = 1;

        [SerializeField]
        private List<uint>
            m_tileIds; //NOTE: name remains for compatibility but now it contains tileData instead of only the id

        public TileSelection() : this(null, 0)
        {
        } //NOTE: fix some errors with serialization
    ...
    public class Tileset : ScriptableObject
    {
        ...
        public const int
            k_BrushId_Default =
                0; // brush id 0 is used for default brush (used by tiles drawn without using a brush asset)

        public const uint k_TileData_Empty = 0xFFFFFFFF;

        // Tile Data Masks
        public const uint
            k_TileDataMask_TileId =
                0x0000FFFF; // up to 256x256(65536 - 1) tiles (in a max. texture size of 8192x8192, min. tile size should be 32x32)

        public const uint k_TileDataMask_BrushId = 0x0FFF0000; // up to 4096 - 1 ( id 0 is used for undefined brush )

        public const uint
            k_TileDataMask_Flags = 0xF0000000; // Flags: (1bit)FlipX, (1bit)FlipY, (1bits)Rot90, (1 bit reserved)
        ...
        public const uint
            k_TileFlag_Updated = 0x10000000; // used by brushes to check when a tile should be updated or not

        public static int GetBrushIdFromTileData(uint tileData)
        {
            return tileData != k_TileData_Empty ? (int) ((tileData & k_TileDataMask_BrushId) >> 16) : -1;
        }

        public static int GetTileIdFromTileData(uint tileData)
        {
            return (int) (tileData & k_TileDataMask_TileId);
        }

        public static uint GetTileFlagsFromTileData(uint tileData)
        {
            return (tileData & k_TileDataMask_Flags);
        }
        ...
        public int Width
        {
            get { return m_tilesetWidth > 0 ? m_tilesetWidth : Mathf.RoundToInt(AtlasTexture.width / TilePxSize.x); }
            set { m_tilesetWidth = Mathf.Clamp(value, 1, Tiles.Count); }
        }
        ...
        public int Height
        {
            get { return 1 + (Tiles.Count - 1) / Width; }
        } //{ get { return m_tilesetHeight > 0 ? m_tilesetHeight : Mathf.RoundToInt(AtlasTexture.height / TilePxSize.y); } }
        ...
        {
            get { return SelectedTileId != k_TileId_Empty ? m_tiles[SelectedTileId] : null; }
        }

        public List<BrushContainer> Brushes
        {
        ...
        {
            get { return m_tileViews; }
        }

        public int BrushTypeMask
        {
            get { return m_brushTypeMask; }
            set { m_brushTypeMask = value; }
        }
        ...
        public int SelectedBrushId
        {
            ...
            set
            {
                ...
                m_selectedBrushId =
                    (int) (m_selectedBrushId & k_TileDataMask_TileId); // convert -1 in k_TileId_Empty            
        ...
        public TileSelection TileSelection
        {
            ...
            {
                m_tileSelection =
                    (m_tileSelection != null && m_tileSelection.selectionData != null &&
                     m_tileSelection.selectionData.Count > 0)
                        ? m_tileSelection
                        : null; //NOTE: sometimes m_tileSelection.tileIds has no tiles, even with the "set" check
                return m_tileSelection;
            }
            set
            {
                TileSelection prevValue = m_tileSelection;
                m_tileSelection = (value != null && value.selectionData != null && value.selectionData.Count > 0)
                    ? value
                    : null;
        ...
        [SerializeField] private List<TileView> m_tileViews = new List<TileView>();
        [SerializeField] private int m_tilesetWidth = 0;
        [SerializeField] private int m_tilesetHeight = 0;
        [SerializeField] private List<BrushContainer> m_brushes = new List<BrushContainer>();
        [SerializeField] private List<Tile> m_tiles = new List<Tile>();
        ...
        [SerializeField] private string[] m_brushGroupNames =
            Enumerable.Range(0, 32).Select(x => x == 0 ? "Default" : "").ToArray();

        [SerializeField]
        private uint[] m_brushGroupAutotilingMatrix = Enumerable.Range(0, 31).Select(x => 1u << x).ToArray();

        [SerializeField] private string[] m_brushTypeMaskOptions;
        [SerializeField] private int m_brushTypeMask = -1;
        ...
        public void AddBrush(TilesetBrush brush)
        {
            if (brush.Tileset == this)
            {
                ...
                {
                    int id = m_brushes.Count > 0
                        ? m_brushes[m_brushes.Count - 1].Id
                        : 1; //NOTE: id 0 is reserved for default brush                
        ...
        public void Slice()
        {
            ...
            if (AtlasTexture != null)
            {
                ...
                if (uInc > 0 && vInc > 0)
                {
                    for (int v = Mathf.RoundToInt(SliceOffset.y);
                        v + TilePxSize.y <= AtlasTexture.height;
                        v += vInc, ++m_tilesetHeight)
                    {
                        for (int u = Mathf.RoundToInt(SliceOffset.x); u + TilePxSize.x <= AtlasTexture.width; u += uInc)
                        {
                            ...
                            {
                                uv = new Rect(
                                    new Vector2((float) u / AtlasTexture.width,
                                        (float) (AtlasTexture.height - v - TilePxSize.y) / AtlasTexture.height),
                                    tileTexselSize)
                            });
                        }
                    }
                    ...
                    if (m_tilesetHeight == 0)
                        Debug.LogError(
                            "No tiles were sliced, maybe because the tile size is greater than the texture size. Tile Height = " +
                            TilePxSize.y + " Texture Height = " + AtlasTexture.height);
                    else
                        m_tilesetWidth = tiles.Count / m_tilesetHeight;
                    TileRowLength = m_tilesetWidth;
                    //Copy data from previous tiles
#if UNITY_EDITOR
                    if (m_tiles != null && m_tiles.Count > 0 && UnityEditor.EditorUtility.DisplayDialog(
                            "Keep previous tile properties?",
                            "Keeping previous tile properties will copy the collider and paramters of previous tiles",
                            "Yes", "No"))
#endif
                    {
                ...
                {
                    Debug.LogWarning(" Error while slicing. There is something wrong with slicing parameters. uInc = " +
                                     uInc + "; vInc = " + vInc);
                }
            }
        }
                        ...
                        if (textureImporter.textureType == UnityEditor.TextureImporterType.Sprite)
                        {
                            if (textureImporter.spriteImportMode == UnityEditor.SpriteImportMode.Multiple)
                            {
                                ...
                                if (textureImporter.spritesheet.Length >= 2)
                                {
                                    ...
                                    SliceOffset = spr0.rect.position;
                                    SliceOffset.y = AtlasTexture.height - spr0.rect.y - spr0.rect.height;
                                ...
                                if (textureImporter.spritesheet.Length >= 2)
                                {
                                    ...
                                    if (m_tiles.Count == 0 ||
                                        !UnityEditor.EditorUtility.DisplayDialog("Import Sprite Sheet?",
                                            "This texture atlas contains sliced sprites. Do you want to overwrite current tiles with these ones?",
                                            "Yes", "No"))
                                        //---
                                    {
                                        m_tilesetHeight = 0;
                                        ...
                                        {
                                            Rect rUV = new Rect(
                                                Vector2.Scale(spriteData.rect.position, AtlasTexture.texelSize),
                                                Vector2.Scale(spriteData.rect.size, AtlasTexture.texelSize));
                                            ...
                                            if (tiles.Count >= 2)
                                            {
                                                if (tiles[tiles.Count - 2].uv.y != tiles[tiles.Count - 1].uv.y)
                                                {
                                                    if (m_tilesetHeight == 1)
                                                    {
                                                        ...
                                                        SlicePadding.y =
                                                            textureImporter.spritesheet[tiles.Count - 2].rect.y -
                                                            textureImporter.spritesheet[tiles.Count - 1].rect.yMax;
                                                    }
                                        ...
                                        if (m_tiles != null && m_tiles.Count > 0 &&
                                            UnityEditor.EditorUtility.DisplayDialog("Keep previous tile properties?",
                                                "Keeping previous tile properties will copy the collider and paramters of previous tiles",
                                                "Yes", "No"))
#endif
                                        {
