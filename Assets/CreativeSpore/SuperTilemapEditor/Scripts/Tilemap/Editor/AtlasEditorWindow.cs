...
namespace CreativeSpore.SuperTilemapEditor
{
    public class AtlasEditorWindow : EditorWindow
    {
        ...
        {
            AtlasEditorWindow wnd =
                (AtlasEditorWindow) EditorWindow.GetWindow(typeof(AtlasEditorWindow), false, "Atlas Editor", true);
        ...
        void OnGUI()
        {
            ...
            m_scrollPos = EditorGUILayout.BeginScrollView(m_scrollPos, GUIStyle.none, GUI.skin.verticalScrollbar);
            {
                ...
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                {
                    ...
                    m_tileset = (Tileset) EditorGUILayout.ObjectField("Tileset (optional)", m_tileset, typeof(Tileset),
                        false);
                    ...
                    m_atlasTexture =
                        (Texture2D) EditorGUILayout.ObjectField("Atlas texture", m_atlasTexture, typeof(Texture2D),
                            false);
                    EditorGUI.indentLevel -= 1;
                }
                ...
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                {
                    ...
                    m_tileSize = EditorGUILayout.Vector2Field("", m_tileSize, GUILayout.MaxWidth(180f),
                        GUILayout.MaxHeight(1f));
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField("Offset", GUILayout.MaxWidth(120f));
                    m_sliceOffset = EditorGUILayout.Vector2Field("", m_sliceOffset, GUILayout.MaxWidth(180f),
                        GUILayout.MaxHeight(1f));
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField("Padding", GUILayout.MaxWidth(120f));
                    m_slicePadding = EditorGUILayout.Vector2Field("", m_slicePadding, GUILayout.MaxWidth(180f),
                        GUILayout.MaxHeight(1f));
                    EditorGUILayout.EndHorizontal();
                    EditorGUI.indentLevel -= 1;
                }
                ...
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                {
                    ...
                    m_padding = EditorGUILayout.IntField(
                        new GUIContent("Padding (pixels)", "Separation between tiles in pixels"), m_padding);
                    m_extrude = EditorGUILayout.IntField(
                    ...
                    if (GUILayout.Button("Preview"))
                    {
                        ...
                        AtlasPreviewWindow atlasPreviewWnd =
                            EditorWindow.GetWindow<AtlasPreviewWindow>(true, "Atlas Preview", true);
                    ...
                    if (GUILayout.Button("Apply Settings"))
                    {
                        ...
                        string atlasTexturePath =
                            Application.dataPath + AssetDatabase.GetAssetPath(m_atlasTexture).Substring(6);
        ...
        public static Texture2D BuildAtlas(Texture2D atlasTexture, int tilePadding, int tileExtrude, Vector2 tileSize,
            Vector2 sliceOffset, Vector2 slicePadding)
        {
            int widthInTiles = Mathf.FloorToInt((atlasTexture.width - sliceOffset.x + slicePadding.x) /
                                                (tileSize.x +
                                                 slicePadding
                                                     .x)); //NOTE: "+ slicePadding.x" makes sure to count the last tile even if no padding pixels are added to the right
            int heightInTiles = Mathf.FloorToInt((atlasTexture.height - sliceOffset.y + +slicePadding.y) /
                                                 (tileSize.y + slicePadding.y));
        ...
        public static Texture2D BuildAtlas(Texture2D atlasTexture, int tilePadding, int tileExtrude, Tileset tileset)
        {
            ...
            Vector2 texselSizeInv =
                new Vector2(1f / tileset.AtlasTexture.texelSize.x, 1f / tileset.AtlasTexture.texelSize.y);
            List<Rect> rects = tileset.Tiles.Select(x =>
                    new Rect(Vector2.Scale(x.uv.position, texselSizeInv), Vector2.Scale(x.uv.size, texselSizeInv)))
                .ToList();
            return BuildAtlas(atlasTexture, tilePadding, tileExtrude, tileset.TilePxSize, widthInTiles, heightInTiles,
                rects);
        }

        public static Texture2D BuildAtlas(Texture2D atlasTexture, int tilePadding, int tileExtrude, Vector2 tileSize,
            int widthInTiles, int heightInTiles, List<Rect> rects)
        {
            ...
            for (int ty = 0, idx = 0; ty < heightInTiles; ++ty)
            {
                for (int tx = 0; tx < widthInTiles; ++tx, ++idx)
                {
                    ...
                    if (tileExtrude > 0)
                    {
                        ...
                        output.SetPixels(dstX - tileExtrude, dstY - tileExtrude, tileExtrude, tileExtrude,
                            paddingColors); // bottom-left corner
                        paddingColors = Enumerable.Repeat(srcTileColors[rw - 1], tileExtrude * tileExtrude).ToArray();
                        output.SetPixels(dstX + rw, dstY - tileExtrude, tileExtrude, tileExtrude,
                            paddingColors); // bottom-right corner
                        paddingColors = Enumerable
                            .Repeat(srcTileColors[srcTileColors.Length - rw], tileExtrude * tileExtrude).ToArray();
                        output.SetPixels(dstX - tileExtrude, dstY + rh, tileExtrude, tileExtrude,
                            paddingColors); // top-left corner
                        paddingColors = Enumerable
                            .Repeat(srcTileColors[srcTileColors.Length - 1], tileExtrude * tileExtrude).ToArray();
                        output.SetPixels(dstX + rw, dstY + rh, tileExtrude, tileExtrude,
                            paddingColors); // top-right corner
                    }
                }
            }
        ...
        public static List<Rect> GenerateGridSpriteRectangles(Texture2D texture, Vector2 offset, Vector2 size,
            Vector2 padding)
        {
        ...
        void OnGUI()
        {
            ...
            m_scrollPos = GUILayout.BeginScrollView(m_scrollPos);
            {
                ...
                for (; tileRect.yMax <= Texture.height; tileRect.y += TileSize.y + Padding)
                {
                    ...
                    {
                        Rect scaledRect = tileRect;
                        scaledRect.position *= m_zoom;
                        scaledRect.size *= m_zoom;
