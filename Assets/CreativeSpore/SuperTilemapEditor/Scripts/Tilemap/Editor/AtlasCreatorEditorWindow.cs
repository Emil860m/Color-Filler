...
namespace CreativeSpore.SuperTilemapEditor
{
    public class AtlasCreatorEditorWindow : EditorWindow
    {
        ...
        [SerializeField] private Texture2D[] m_tileTextures = null;
        ...
        {
            AtlasCreatorEditorWindow wnd =
                (AtlasCreatorEditorWindow) EditorWindow.GetWindow(typeof(AtlasCreatorEditorWindow), false,
                    "Atlas Creator", true);
        ...
        void OnGUI()
        {
            ...
            m_scrollPos = EditorGUILayout.BeginScrollView(m_scrollPos, GUIStyle.none, GUI.skin.verticalScrollbar);
            {
                ...
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                {
                    ...
                    m_maxTextureSize =
                        (eMaxTextureSize) EditorGUILayout.EnumPopup("Max. Texture Size", m_maxTextureSize);
                    if (m_tileset && GUILayout.Button("Create Atlas & Update Tileset") ||
                        !m_tileset && GUILayout.Button("Create Atlas"))
                    {
                        if (!m_atlasTexture)
                            m_atlasTexture = new Texture2D((int) m_maxTextureSize, (int) m_maxTextureSize,
                        ...
                        Rect[] tileRects =
                            m_atlasTexture.PackTextures(m_tileTextures, m_padding, (int) m_maxTextureSize, false);
                        ...
                        {
                            Texture2D tempAtlas = new Texture2D(m_atlasTexture.width, m_atlasTexture.height,
                                TextureFormat.ARGB32, false);
                        ...
                        if (string.IsNullOrEmpty(atlasAssetPath))
                        {
                            ...
                            atlasAssetPath = EditorUtility.SaveFilePanelInProject("Save atlas texture", name + "Atlas",
                                "png", "Save the atlas texture");
                        }
                        ...
                        if (!string.IsNullOrEmpty(atlasAssetPath))
                        {
                            ...
                            if (m_tileset && tileRects.Length > 0)
                            {
                                ...
                                m_tileset.TilePxSize = new Vector2(tileRects[0].size.x * m_atlasTexture.width,
                                    tileRects[0].size.y * m_atlasTexture.height);
                                int idx;
