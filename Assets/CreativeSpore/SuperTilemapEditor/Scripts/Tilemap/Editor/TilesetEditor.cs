...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilesetEditor : Editor
    {
        ...
        public virtual void OnEnable()
        {
            ...
            {
                this.m_groupsList = new ReorderableList(base.serializedObject, this.m_brushGroupNames, false, false,
                    false, false);
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                if (GUILayout.Button(new GUIContent("Optimize Atlas Texture Settings",
                    "Change the import settings for an optimum result")))
                {
                ...
                if (s_atlasSettingsFoldout)
                {
                    ...
                    tileset.TilePxSize =
                        _GetPositiveIntVect2(EditorGUILayout.Vector2Field("Tile Size (pixels)", tileset.TilePxSize),
                            Vector2.one);
                    tileset.SliceOffset = _GetPositiveIntVect2(EditorGUILayout.Vector2Field(
                        new GUIContent("Offset",
                            "The offset position in pixels to start slicing tiles from the atlas texture"),
                        tileset.SliceOffset));
                    tileset.SlicePadding = _GetPositiveIntVect2(EditorGUILayout.Vector2Field(
                        new GUIContent("Padding", "The separation in pixels between tiles"), tileset.SlicePadding));
                ...
                EditorGUILayout.BeginVertical(EditorStyles.helpBox,
                    GUILayout.MinHeight(s_mode == eMode.Tiles ? Screen.height * 0.8f : 0f));
                {
                    switch (s_mode)
                    {
                        case eMode.Tiles:
                        ...
                        case eMode.BrushGroups:
                            ...
                            GroupMatrixGUI.DoGUI("Group Autotiling Mask", tileset.BrushGroupNames,
                                ref s_brushAutotilingMaskFoldout, ref s_brushGroupMatrixScrollPos, GetAutotiling,
                                SetAutotiling);
                            EditorGUILayout.HelpBox(
                                "Check the group that should autotile between them when Autotiling Mode Group is enabled in a brush",
                                MessageType.Info);
                            break;
                    }
                }
        ...
        {
            ReorderableList tileViewRList =
                new ReorderableList(tileset.TileViews, typeof(TileView), true, true, true, true);
            tileViewRList.onAddDropdownCallback = (Rect buttonRect, ReorderableList l) =>
            {
                ...
                GenericMenu.MenuFunction addTileSelectionFunc = () =>
                {
                    ...
                    tileSelection
                        .FlipVertical(); // flip vertical to fit the tileset coordinate system ( from top to bottom )                   
                ...
                {
                    if (EditorUtility.DisplayDialog("Warning!", "Are you sure you want to delete all the TileViews?",
                        "Yes", "No"))
                    {
            ...
            {
                if (EditorUtility.DisplayDialog("Warning!", "Are you sure you want to delete the TileView?", "Yes",
                    "No"))
                {
            ...
            tileViewRList.drawHeaderCallback = (Rect rect) =>
            {
                ...
                Texture2D btnTexture = tileViewRList.elementHeight == 0f
                ...
                if (GUI.Button(new Rect(rect.width - rect.height, rect.y, rect.height, rect.height), btnTexture,
                    EditorStyles.label))
                {
                    tileViewRList.elementHeight =
                        tileViewRList.elementHeight == 0f ? EditorGUIUtility.singleLineHeight : 0f;
        ...
        {
            ReorderableList brushRList =
                new ReorderableList(tileset.Brushes, typeof(Tileset.BrushContainer), true, true, true, true);
            ...
            brushRList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                if (brushContainer.BrushAsset)
                {
                    ...
                    Rect rTileId = rTile;
                    rTileId.x += rTile.width + 20;
                    rTileId.width = rect.width - rTileId.x;
                    ...
                    brushContainer.BrushAsset.ShowInPalette = EditorGUI.Toggle(rToggle, GUIContent.none,
                        brushContainer.BrushAsset.ShowInPalette, STEditorStyles.Instance.visibleToggleStyle);
        ...
        public static void DoGUIDrawTileFromTileData(Rect dstRect, uint tileData, Tileset tileset,
            Rect customUV = default(Rect))
        {
                ...
                if (tileId != Tileset.k_TileId_Empty && tileset.AtlasTexture)
                {
                    ...
                    if (tile != null && tile.prefabData.prefab && tile.prefabData.showPrefabPreviewInTilePalette)
                    {
                        ...
                        if (assetPreview)
                            ...
                            GUI.DrawTextureWithTexCoords(dstRect, tileset.AtlasTexture,
                                customUV == default(Rect) && tile != null ? tile.uv : customUV, true);
                    }
                    else
                    {
                        GUI.DrawTextureWithTexCoords(dstRect, tileset.AtlasTexture,
        ...
        public static void FixTextureSize(Texture2D tex, TextureImporter importer)
        {
            ...
            {
                EditorUtility.DisplayDialog("Texture is too big",
                    "The texture " + tex.name + " is too big. The width and height should be less or equal to " +
                    textureSizes[textureSizes.Length - 2], "Ok");
            }
            else
            {
    ...
    internal class GroupMatrixGUI
    {
        ...
        public static void DoGUI(string title, string[] groupNames, ref bool show, ref Vector2 scrollPos,
            GetValueFunc getValue, SetValueFunc setValue, bool allowBothCombinations = true)
        {
            int num = 0;
            ...
            if (show)
            {
                ...
                Vector2 vector =
                    GUIClip_Unclip(new Vector2(rect.x, rect.y)); //GUIClip.Unclip(new Vector2(rect.x, rect.y));
                int num2 = 0;
                for (int j = 0; j < groupNames.Length; j++)
                {
                    if (groupNames[j] != string.Empty)
                    {
                        ...
                        Vector3 pos =
                            new Vector3((float) (130 + 16 * (num - num2)) + vector.y + vector.x + scrollPos.y - num3,
                                vector.y + scrollPos.y, 0f);
                        GUI.matrix = Matrix4x4.TRS(pos, Quaternion.identity, Vector3.one) *
                        ...
                        {
                            GUI.matrix *= Matrix4x4.TRS(new Vector3(-0.5f, -0.5f, 0f), Quaternion.identity,
                                Vector3.one);
                        }

                        GUI.Label(new Rect(2f - vector.x - scrollPos.y, scrollPos.y - num3, 100f, 16f), groupNames[j],
                            "RightLabel");
                        num2++;
                    }
                }
                            ...
                            if (groupNames[l] != string.Empty)
                            {
                                //if (num4 < num - num2)
                                {
                                    GUIContent content = new GUIContent(string.Empty,
                                        groupNames[k] + "/" + groupNames[l]);
                                    bool flag = getValue(k, l);
                                    bool flag2 =
                                        GUI.Toggle(new Rect(130f + rect2.x + (float) (num4 * 16), rect2.y, 16f, 16f),
                                            flag, content);
                                    if (flag2 != flag)
                                    {
                                        setValue(k, l, flag2);
        ...
        private static void Initialize()
        {
            if (s_GUIClip_Type == null)
            {
                ...
                if (s_GUIClip_Type != null)
                {
                    s_GUIClip_topmostRect_Property = s_GUIClip_Type.GetProperty("topmostRect",
