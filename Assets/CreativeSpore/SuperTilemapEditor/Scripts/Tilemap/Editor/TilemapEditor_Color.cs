    ...
    public partial class TilemapEditor
    {
        private class ColorSettings
        {
            ...
            public AnimationCurve
                brushIntensity =
                    AnimationCurve.EaseInOut(0f, 0f, 1f,
                        1f); // new AnimationCurve(new Keyframe[] { new Keyframe(0f, 0f, 2f, 2f), new Keyframe(1f, 1f) });
        }
        ...
        private void OnInspectorGUI_Color()
        {
            ...
            s_colorSettings.tileColorPaintMode = (eTileColorPaintMode) EditorGUILayout.EnumPopup(
                new GUIContent("Tile Color Paint Mode",
                    "Tile mode will change the 4 vertices of the tile using the same color; Vertex will take into account each vertex of the tile separately."),
                s_colorSettings.tileColorPaintMode);
            if (m_tilemap.ParentTilemapGroup)
            {
                s_colorSettings.paintTilemapGroup = EditorGUILayout.Toggle(
                    new GUIContent("Paint Tilemap Group", "Paints all the tilemaps in the tilemap group."),
                    s_colorSettings.paintTilemapGroup);
            }

            s_colorSettings.radius = Mathf.Max(0f, EditorGUILayout.FloatField("Radius", s_colorSettings.radius));
            s_colorSettings.brushIntensity = EditorGUILayout.CurveField(
                new GUIContent("Brush Intensity",
                    "The alpha of the color will be multiplied by this curve values along the radius."),
                s_colorSettings.brushIntensity);
            ...
            s_enableUndoColorPainting = EditorGUILayout.Toggle(
                new GUIContent("Enable Undo",
                    "Enables undo/redo for color paint actions. It could slowdown the color painting actions."),
                s_enableUndoColorPainting);
            EditorGUILayout.Space();
            string helpInfo =
                    "- Hold " + EditorCompatibilityUtils.CtrKeyName +
                    " while scrolling up/down to change the brush radius.\n" +
                    "- Hold " + EditorCompatibilityUtils.AltKeyName +
                    " while pressing the left mouse button to pick tile color.\n"
                ;
                ...
                if (!skip)
                {
                    ...
                    GUIUtility.hotControl = controlID;
                    {
                        ...
                        if (chunkPlane.Raycast(ray, out dist))
                        {
                            ...
                            EditorCompatibilityUtils.CircleCap(0, brushWorldPos, m_tilemap.transform.rotation,
                                s_colorSettings.radius);
                            if (
                            ...
                            )
                            {
                                if (e.button == 0)
                                {
                                    ...
                                    else if (s_colorSettings.paintTilemapGroup && m_tilemap.ParentTilemapGroup)
                                    {
                                        m_tilemap.ParentTilemapGroup.IterateTilemapWithAction((STETilemap tilemap) =>
                                        {
                                            if (tilemap && tilemap.IsVisible)
                                            {
                                                if (s_enableUndoColorPainting)
                                                    RegisterTilemapUndo(tilemap);
                                                TilemapVertexPaintUtils.VertexPaintCircle(tilemap, m_localBrushPos,
                                                    s_colorSettings.radius, s_colorSettings.color,
                                                    s_colorSettings.blendMode,
                                                    s_colorSettings.tileColorPaintMode == eTileColorPaintMode.Vertex,
                                                    s_colorSettings.brushIntensity);
                                                tilemap.UpdateMesh();
                                            }
                                        });
                                    ...
                                    else
                                    {
                                        if (s_enableUndoColorPainting)
                                            RegisterTilemapUndo(m_tilemap);
                                        TilemapVertexPaintUtils.VertexPaintCircle(m_tilemap, m_localBrushPos,
                                            s_colorSettings.radius, s_colorSettings.color, s_colorSettings.blendMode,
                                            s_colorSettings.tileColorPaintMode == eTileColorPaintMode.Vertex,
                                            s_colorSettings.brushIntensity);
                                        m_tilemap.UpdateMesh();
                                    }
