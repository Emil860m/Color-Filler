...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public partial class TilemapEditor : Editor
    {
        ...
        public static eEditMode EditMode
        {
        ...
        [SerializeField] private bool m_toggleMapBoundsEdit = false;

        private STETilemap m_tilemap;

        private Tileset
            m_tilemapTileset; //NOTE: needed to Unregister tileset events. When tilemap is destroyed, m_tilemap is nulled during OnDisable
        ...
        public override void OnInspectorGUI()
        {
            ...
            m_tilemap.Tileset =
                (Tileset) EditorGUILayout.ObjectField("Tileset", m_tilemap.Tileset, typeof(Tileset), false);
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox,
                GUILayout.MinHeight(s_editMode == eEditMode.Paint ? Screen.height * 0.8f : 0f));
            {
        ...
        public void OnSceneGUI()
        {
            ...
            m_tilemap.transform.localPosition += (Vector3) (Vector2.Scale(Camera.current.transform.position,
                (Vector2.one - m_tilemap.ParallaxFactor))); //apply parallax
        ...
        private void OnInspectorGUI_Collider()
        {
            EditorGUI.BeginChangeCheck();
            {
                ...
                string[] colliderTypeNames =
                    new List<string>(System.Enum.GetNames(typeof(eColliderType)).Select(x => x.Replace('_', ' ')))
                        .ToArray();
                ...
                if (m_tilemap.ColliderType == eColliderType._3D)
                {
                    ...
                    m_ColliderDepth.floatValue = Mathf.Clamp(m_ColliderDepth.floatValue, Vector3.kEpsilon,
                        Mathf.Max(m_ColliderDepth.floatValue));
                ...
                if (m_tilemap.IsTrigger)
                {
                    EditorGUILayout.HelpBox(
                        ...
                        MessageType.Warning);
                         ...
                         m_tilemap.Collider2DType == e2DColliderType.PolygonCollider2D)
                {
                    EditorGUILayout.HelpBox(
                        "Using Polygon colliders could generate wrong collider lines if not used properly. Use Show Tilechunks in Map section to display the real collider lines.",
                        MessageType.Warning);
                }
            }
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                EditorGUI.indentLevel += 2;
                EditorGlobalSettings.TilemapColliderColor =
                    EditorGUILayout.ColorField("Collider Color", EditorGlobalSettings.TilemapColliderColor);
                EditorGUI.indentLevel -= 2;
            }
            EditorGUILayout.EndVertical();
        }
        ...
        private void OnInspectorGUI_Map()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button(
                    new GUIContent("R",
                        "Reset the CellSize to the default value in base to the Tileset properties: PixelsPerUnit and TileSize"),
                    GUILayout.Width(20)))
                {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                m_toggleMapBoundsEdit = EditorUtils.DoToggleIconButton("Edit Map Bounds", m_toggleMapBoundsEdit,
            ...
            m_tilemap.AllowPaintingOutOfBounds =
                EditorGUILayout.ToggleLeft("Allow Painting Out of Bounds", m_tilemap.AllowPaintingOutOfBounds);
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                isChunksVisible =
                    EditorGUILayout.Toggle(
                        new GUIContent("Show Tile Chunks",
                            "Show tilemap chunk objects for debugging or other purposes. Hiding will be refreshed after collapsing the tilemap."),
                        isChunksVisible);
                if (EditorGUI.EndChangeCheck())
                {
                ...
                EditorGUILayout.PropertyField(m_enableUndoWhilePainting,
                    new GUIContent("Enable Undo", "Disable Undo when painting on big maps to improve performance."));
            }
            EditorGUILayout.EndVertical();
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                EditorGUILayout.LabelField("Global Parameters:", EditorStyles.boldLabel);
                EditorGUI.indentLevel += 2;
                EditorGlobalSettings.TilemapGridColor =
                    EditorGUILayout.ColorField("Grid Color", EditorGlobalSettings.TilemapGridColor);
                EditorGUI.indentLevel -= 2;
            }
            EditorGUILayout.EndVertical();
        }
        ...
        private void OnInspectorGUI_Renderer()
        {
            ...
            */
            ...
            if (EditorGUI.EndChangeCheck())
            {
                m_orderInLayer.intValue =
                    (m_orderInLayer.intValue << 16) >> 16; // convert from int32 to int16 keeping sign
                serializedObject.ApplyModifiedProperties();
            ...
            if (EditorGUI.EndChangeCheck())
                m_tilemap.Refresh(true, false, false, false);
            EditorGUILayout.PropertyField(m_InnerPadding,
                new GUIContent("Inner Padding",
            ...
            if (GUILayout.Button("Export To Png file"))
            {
                ...
                string path = EditorUtility.SaveFilePanel("Save Png file...", AssetDatabase.GetAssetPath(m_tilemap),
                    defaultName + ".png", "png");
                if (!string.IsNullOrEmpty(path))
                {
                    Texture2D previewTexture =
                        TilemapUtils.CreateTilemapGroupPreviewTexture(m_tilemap, m_tilemap.ParentTilemapGroup);
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                if (m_tilemap.ChunkRendererProperties.reflectionProbeUsage !=
        ...
        private void DoPaintSceneGUI()
        {
            ...
            if (DoToolBar()
                ...
                SceneView.currentDrawingSceneView // hide brush when it's not over the scene view
                ...
                if (!skip)
                {
                    if (e.type == EventType.KeyDown)
                    {
                        switch (e.keyCode)
                        {
                            ...
                            case ShortcutKeys.k_EllipseTool:
                                ...
                                {
                                    case ShortcutKeys.k_PencilTool:
                                        BrushBehaviour.Instance.PaintMode = BrushBehaviour.eBrushPaintMode.Pencil;
                                        break;
                                    case ShortcutKeys.k_LineTool:
                                        BrushBehaviour.Instance.PaintMode = BrushBehaviour.eBrushPaintMode.Line;
                                        break;
                                    case ShortcutKeys.k_RectTool:
                                        BrushBehaviour.Instance.PaintMode =
                                            BrushBehaviour.Instance.PaintMode == BrushBehaviour.eBrushPaintMode.Rect
                                                ? BrushBehaviour.eBrushPaintMode.FilledRect
                                                : BrushBehaviour.eBrushPaintMode.Rect;
                                        break;
                                    case ShortcutKeys.k_EllipseTool:
                                        BrushBehaviour.Instance.PaintMode =
                                            BrushBehaviour.Instance.PaintMode == BrushBehaviour.eBrushPaintMode.Ellipse
                                                ? BrushBehaviour.eBrushPaintMode.FilledEllipse
                                                : BrushBehaviour.eBrushPaintMode.Ellipse;
                                        break;
                                }

                                s_brushMode = eBrushMode.Paint;
                                STEditorToolbars.Instance.brushToolbar.TriggerButton((int) s_brushMode);
                                STEditorToolbars.Instance.brushPaintToolbar.TriggerButton(
                                    (int) BrushBehaviour.Instance.PaintMode);
                                e.Use();
                                break;
                        }
                    }
                    ...
                    GUIUtility.hotControl = controlID;
                    {
                        ...
                        if (chunkPlane.Raycast(ray, out dist))
                        {
                            ...
                            Vector2 startPos = new Vector2(Mathf.Min(m_startDragging.x, m_endDragging.x),
                                Mathf.Min(m_startDragging.y, m_endDragging.y));
                            Vector2 endPos = new Vector2(Mathf.Max(m_startDragging.x, m_endDragging.x),
                                Mathf.Max(m_startDragging.y, m_endDragging.y));
                            Vector2 selectionSnappedPos = BrushUtil.GetSnappedPosition(startPos, m_tilemap.CellSize);
                            Vector2 selectionSize = BrushUtil.GetSnappedPosition(endPos, m_tilemap.CellSize) -
                                                    selectionSnappedPos + m_tilemap.CellSize;

                            BrushBehaviour brush = BrushBehaviour.GetOrCreateBrush(m_tilemap);
                            // Update brush transform
                            m_localBrushPos = (Vector2) m_tilemap.transform.InverseTransformPoint(ray.GetPoint(dist));
                            Vector2 brushSnappedPos =
                                BrushUtil.GetSnappedPosition(brush.Offset + m_localBrushPos, m_tilemap.CellSize);
                            ...
                            if (!BrushBehaviour.Instance.IsDragging)
                            {
                                brush.transform.position =
                                    m_tilemap.transform.TransformPoint(new Vector3(brushSnappedPos.x, brushSnappedPos.y,
                                        -0.01f));
                            }
                            //---
                            ...
                            {
                                m_fillPreview.Clear();
                                TilemapDrawingUtils.FloodFillPreview(m_tilemap, brush.Offset + m_localBrushPos,
                                    brush.BrushTilemap.GetTileData(0, 0), m_fillPreview);
                            }

                            bool isModifiersChanged = false;
                            if (e.isKey)
                            {
                                EventModifiers filteredModifiers =
                                    e.modifiers & (EventModifiers.Control | EventModifiers.Alt);
                            ...
                            if (
                                ...
                                (e.type == EventType.MouseDown || e.type == EventType.MouseDrag && isMouseGridChanged ||
                                 e.type == EventType.MouseUp || isModifiersChanged & GetBrushMode() == eBrushMode.Paint)
                            )
                            {
                                if (e.button == 0)
                                {
                                    if (m_dblClick.IsDblClick && brush.BrushTilemap.GridWidth == 1 &&
                                        brush.BrushTilemap.GridHeight == 1)
                                    {
                                        // Restore previous tiledata modified by Paint, because before the double click, a single click is done before
                                        m_tilemap.SetTileData(brush.Offset + m_localBrushPos,
                                            m_floodFillRestoredTileData);
                                    ...
                                    else
                                    {
                                        switch (GetBrushMode())
                                        {
                                            case eBrushMode.Paint:
                                                ...
                                                {
                                                    m_floodFillRestoredTileData =
                                                        m_tilemap.GetTileData(m_mouseGridX, m_mouseGridY);
                                                }

                                                if (e.type == EventType.MouseDown)
                                                    brush.DoPaintPressed(m_tilemap, brush.Offset + m_localBrushPos,
                                                        e.modifiers);
                                                else if (e.type == EventType.MouseDrag || isModifiersChanged &&
                                                         BrushBehaviour.Instance.PaintMode !=
                                                         BrushBehaviour.eBrushPaintMode.Pencil)
                                                    brush.DoPaintDragged(m_tilemap, brush.Offset + m_localBrushPos,
                                                        e.modifiers);
                                                else if (e.type == EventType.MouseUp)
                                                    brush.DoPaintReleased(m_tilemap, brush.Offset + m_localBrushPos,
                                                        e.modifiers);
                                                break;
                            ...
                            if (e.type == EventType.MouseUp)
                            {
                                if (e.button == 1) // right mouse button
                                {
                                    ...
                                    ResetBrushMode();
                                    // Copy one tile
                                    if (selectionSize.x <= m_tilemap.CellSize.x &&
                                        selectionSize.y <= m_tilemap.CellSize.y)
                                    {
                                        ...
                                        if (e.control && m_tilemap.ParentTilemapGroup)
                                        {
                                            ...
                                            for (int i = 0;
                                                i < m_tilemap.ParentTilemapGroup.Tilemaps.Count;
                                        ...
                                        else
                                        {
                                            ...
                                            if (brushId > 0 && !e.alt
                                            ) //NOTE: if Alt is held, the tile is selected instead
                                            {
                                            ...
                                            else
                                            {
                                                ...
                                                brush.BrushTilemap.SetTileData(0, 0,
                                                    tileData & ~Tileset.k_TileDataMask_BrushId); // keep tile flags
                                            }
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        brush.Offset.x = m_endDragging.x > m_startDragging.x
                                            ? -(endGridX - startGridX) * m_tilemap.CellSize.x
                                            : 0f;
                                        brush.Offset.y = m_endDragging.y > m_startDragging.y
                                            ? -(endGridY - startGridY) * m_tilemap.CellSize.y
                                            : 0f;
                                    }
                                }
                            }
                            ...
                            {
                                Rect rBound = new Rect(brush.BrushTilemap.MapBounds.min,
                                    brush.BrushTilemap.MapBounds.size);
                                Color fillColor;
                                switch (GetBrushMode())
                                {
                                    case eBrushMode.Paint:
                                ...
                                HandlesEx.DrawRectWithOutline(brush.transform, rBound, fillColor,
                                    new Color(1, 1, 1, 0.2f));
                            }
                        }
                    }

                    if (currentEventType == EventType.MouseDrag && Event.current.button < 2
                    ) // 2 is for central mouse button
                    {
        ...
        private void DoMapSceneGUI()
        {
            if (m_toggleMapBoundsEdit)
            {
                ...
                Vector3 vMinX = Handles.FreeMoveHandle(
                    m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MinGridX * m_tilemap.CellSize.x,
                        m_tilemap.MapBounds.center.y)), Quaternion.identity,
                    0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero,
                    EditorCompatibilityUtils.CubeCap);
                Vector3 vMaxX = Handles.FreeMoveHandle(
                    m_tilemap.transform.TransformPoint(new Vector2((m_tilemap.MaxGridX + 1f) * m_tilemap.CellSize.x,
                        m_tilemap.MapBounds.center.y)), Quaternion.identity,
                    0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero,
                    EditorCompatibilityUtils.CubeCap);
                Vector3 vMinY = Handles.FreeMoveHandle(
                    m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x,
                        m_tilemap.MinGridY * m_tilemap.CellSize.y)), Quaternion.identity,
                    0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero,
                    EditorCompatibilityUtils.CubeCap);
                Vector3 vMaxY = Handles.FreeMoveHandle(
                    m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x,
                        (m_tilemap.MaxGridY + 1f) * m_tilemap.CellSize.y)), Quaternion.identity,
                    0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero,
                    EditorCompatibilityUtils.CubeCap);
                Handles.color = Color.white;
                int minGridX =
                    Mathf.RoundToInt(m_tilemap.transform.InverseTransformPoint(vMinX).x / m_tilemap.CellSize.x);
                int maxGridX =
                    Mathf.RoundToInt(m_tilemap.transform.InverseTransformPoint(vMaxX).x / m_tilemap.CellSize.x - 1f);
                int minGridY =
                    Mathf.RoundToInt(m_tilemap.transform.InverseTransformPoint(vMinY).y / m_tilemap.CellSize.y);
                int maxGridY =
                    Mathf.RoundToInt(m_tilemap.transform.InverseTransformPoint(vMaxY).y / m_tilemap.CellSize.y - 1f);
                if (EditorGUI.EndChangeCheck())
                {
                    EditorUtility.SetDirty(target);
        ...
        private static readonly Vector2[] s_fullCollTileVertices = new Vector2[]
        ...
        private void DoColliderSceneGUI()
        {
            ...
            if (EditorWindow.mouseOverWindow == SceneView.currentDrawingSceneView)
            {
                    ...
                    GetMousePosOverTilemap(m_tilemap, out vLocMousePos))
                {
                    ...
                    Vector2 vTileCenter = BrushUtil.GetSnappedPosition(vLocMousePos, m_tilemap.CellSize) +
                                          m_tilemap.CellSize / 2f;
                    float distX = Mathf.Abs(vLocMousePos.x - vTileCenter.x);
                    float distY = Mathf.Abs(vLocMousePos.y - vTileCenter.y);
                    if (distX <= m_tilemap.CellSize.x * threshold / 2f &&
                        distY <= m_tilemap.CellSize.y * threshold / 2f)
                ...
                if (tile != null)
                {
                    ...
                    HandlesEx.DrawRectWithOutline(m_tilemap.transform,
                        new Rect(BrushUtil.GetSnappedPosition(m_colMouseLocPos, m_tilemap.CellSize),
                            m_tilemap.CellSize), new Color(0f, 0f, 0f, 0.1f), new Color(0f, 0f, 0f, 0f));
                    ...
                    List<Vector2> handlesPos =
                        new List<Vector2>(tile.collData.vertices != null && tile.collData.vertices.Length > 0
                            ? tileColliderData.vertices
                            : s_fullCollTileVertices);
                    for (int i = 0; i < handlesPos.Count; ++i)
                        handlesPos[i] =
                            m_tilemap.transform.TransformPoint(
                                tileLocPos + Vector2.Scale(handlesPos[i], m_tilemap.CellSize));
                    ...
                    if (e.alt)
                    {
                        ...
                        if (Handles.Button(centerPos, Quaternion.identity, size, m_tilemap.CellSize.x,
                            EditorCompatibilityUtils.CircleCap))
                        {
                            Undo.RecordObject(m_tilemap.Tileset, "Update Tile Collider");
                            tile.collData.type = tile.collData.type == eTileCollider.None
                                ? eTileCollider.Polygon
                                : eTileCollider.None;
                    ...
                    for (int i = 0; i < handlesPos.Count; ++i)
                    {
                        ...
                        Vector2 vLocVertex =
                            (Vector2) m_tilemap.transform.InverseTransformPoint(handlePos) - tileLocPos;
                        Vector2 oldVertexValue = PointToSnappedVertex(vLocVertex, m_tilemap);
                        if (tile.collData.type != eTileCollider.None)
                        {
                            handlePos = Handles.FreeMoveHandle(handlePos, Quaternion.identity,
                                0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero,
                                EditorCompatibilityUtils.CubeCap);
                            if (closestVertIdx >= 0 && i == closestVertIdx + 1)
                                HandlesEx.DrawDotOutline(handlePos,
                                    0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Color.green,
                                    Color.green);
                            else if (e.control && HandleUtility.DistanceToCircle(handlePos, 0f) < 10f)
                            {
                                selectedIdx = idx;
                                HandlesEx.DrawDotOutline(handlePos,
                                    0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Color.red,
                                    Color.red);
                            }
                            ...
                            updateColliders |= (oldVertexValue != newVertexValue) || m_lastControl == 0 &&
                                               EditorGUIUtility.hotControl != 0 &&
                                               HandleUtility.DistanceToCircle(handlePos, 0f) < 10f;
                            if (updateColliders)
                            {
                                Undo.RecordObject(m_tilemap.Tileset, "Update Tile Collider");
                                handlesPos[i] = handlePos;
                                tile.collData.type = eTileCollider.Polygon;
                                tile.collData.vertices = handlesPos.Select(x =>
                                        PointToSnappedVertex(
                                            (Vector2) m_tilemap.transform.InverseTransformPoint(x) - tileLocPos,
                                            m_tilemap))
                                    .ToArray();
                        ...
                        {
                            HandlesEx.DrawDotOutline(handlePos,
                                0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Color.gray * 0.2f,
                                Color.gray * 0.2f);
                        }
                    ...
                    if (e.control && HandleUtility.nearestControl == EditorGUIUtility.hotControl &&
                        tile.collData.type != eTileCollider.None)
                    {
                        EditorGUIUtility.hotControl = 0;
                    ...
                    if (tile.collData.type == eTileCollider.Polygon &&
                        Enumerable.SequenceEqual(tile.collData.vertices, s_fullCollTileVertices))
            ...
            string helpInfo =
                "<b>" +
                "<color=#FFD500FF>  - Hold " +
                ((Application.platform == RuntimePlatform.OSXEditor) ? "Option" : "Alt") +
                " + Click to enable/disable tile colliders" + "</color>\n" +
                "<color=orange>" +
                "  - Hold Shift + Click to add a new vertex" + "\n" +
                "  - Hold " + ((Application.platform == RuntimePlatform.OSXEditor) ? "Command" : "Ctrl") +
                " + Click to remove a vertex. (should be more than 3)" + "\n" +
        ...
        bool DoToolBar()
        {
            ...
            if (m_tilemap.Tileset.SelectedTileId != Tileset.k_TileId_Empty)
                selectedTileOrBrushId =
                    new GUIContent("<b> Selected Tile Id: " + m_tilemap.Tileset.SelectedTileId.ToString() + "</b>");
            else if (m_tilemap.Tileset.SelectedBrushId != Tileset.k_BrushId_Default)
                selectedTileOrBrushId =
                    new GUIContent("<b> Selected Brush Id: " + m_tilemap.Tileset.SelectedBrushId.ToString() + "</b>");
            else
                selectedTileOrBrushId = new GUIContent("<b> Empty tile selected</b>");

            Rect rTools = new Rect(4f, 4f,
                Mathf.Max(
                    Mathf.Max(Styles.Instance.toolbarBoxStyle.CalcSize(brushCoords).x,
                        Styles.Instance.toolbarBoxStyle.CalcSize(selectedTileOrBrushId).x) + 4f, 180f), 54f);

            Handles.BeginGUI();
            GUILayout.BeginArea(rTools);
            HandlesEx.DrawRectWithOutline(new Rect(Vector2.zero, rTools.size), s_toolbarBoxBgColor,
                s_toolbarBoxOutlineColor);
            ...
            STEditorToolbars.Instance.brushToolbar.SetHighlight((int) ToolIcons.eToolIcon.Erase,
                GetBrushMode() == eBrushMode.Erase);
            STEditorToolbars.Instance.brushToolbar.DoGUI(toolbarPos, toolbarButtonSize, s_toolbarBoxBgColor,
                s_toolbarBoxOutlineColor);
            //---     
            ...
            if (Tools.current != Tool.None && Tools.current != Tool.Rect)
            {
                ...
                EditorGUI.HelpBox(new Rect(Vector2.zero, rWarningArea.size),
                    "Select the Rect Tool (T) or press any toolbar button to start painting", MessageType.Warning);
                GUILayout.EndArea();
            }
            else if (s_brushMode == eBrushMode.Paint)
            {
                ...
                STEditorToolbars.Instance.brushPaintToolbar.DoGUI(toolbarPos, toolbarButtonSize,
                    s_toolbarBoxBgColor * 1.4f, s_toolbarBoxOutlineColor * 1.4f);
                //---
            }
        ...
        void DisplayHelpBox()
        {
            ...
            HandlesEx.DrawRectWithOutline(new Rect(Vector2.zero, rHelpBox.size), s_toolbarBoxBgColor,
                s_toolbarBoxOutlineColor);
