    ...
    public class TilemapGroupEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("m_unselectedColorMultiplier"),
                new GUIContent("Highlight Alpha"));
            EditorGUIUtility.labelWidth = 200f;
            EditorGUILayout.PropertyField(m_defaultTilemapWindowVisibleProperty,
                new GUIContent("Display Default Tilemap Window"));
        ...
        public void OnSceneGUI()
        {
            ...
            m_sceneViewTilemapRList.DoList(new Rect(Screen.width - 300f - 10f,
                Screen.height - m_sceneViewTilemapRList.GetHeight() - hOff, 300f, 1f));
            ...
            {
                Rect rDefaultTilemapWindow = new Rect(5f, Screen.height - m_defaultTilemapsWindowRect.height - 20f,
                    m_defaultTilemapsWindowRect.width, m_defaultTilemapsWindowRect.height);
                if (m_defaultTilemapWindowMinimized)
                {
                    rDefaultTilemapWindow.y = Screen.height - 55f;
                }

                GUI.Window(0, rDefaultTilemapWindow, DoWindow, "Tile/Brush Default Tilemap",
                    STEditorStyles.Instance.boldWindow);
            }
        }
        ...
        private void DoWindow(int id)
        {
            ...
            GUILayout.BeginArea(new Rect(0f, EditorGUIUtility.singleLineHeight, m_defaultTilemapsWindowRect.width,
                m_defaultTilemapsWindowRect.height));
            GUILayout.Label("Default Tilemap: " + (defaultTilemap ? "<b>" + defaultTilemap.name + "</b>" : "<none>"),
                STEditorStyles.Instance.richLabel);
            if (GUILayout.Button("Set Default Tilemap <b>(For the Palette Selection)</b>",
                STEditorStyles.Instance.richButton))
            {
                if (m_target.SelectedTilemap && m_target.SelectedTilemap.Tileset &&
                    m_target.SelectedTilemap.Tileset.TileSelection != null)
                {
            ...
            if (GUILayout.Button("Remove Default Tilemap <b>(For the Palette Selection)</b>",
                STEditorStyles.Instance.richButton))
                m_target.ClearDefaultTilemapForCurrentSelectedTileOrBrush();
            EditorGUILayout.Space();
            if (GUILayout.Button("Set Default Tilemap <b>(For the Tiles in the Tilemap)</b>",
                STEditorStyles.Instance.richButton))
            ...
            */
            if (GUILayout.Button("Clear Default Tilemap Data <b>(For all Tiles)</b>",
                STEditorStyles.Instance.richButton))
        ...
        private void DoKeyboardChecks()
        {
            ...
            if (e.type == EventType.KeyDown)
            {
                ...
                else if (e.keyCode == ShortcutKeys.k_PrevLayer)
                    m_selectedIndexProp.intValue += m_tilemapReordList.count - 1;
        ...
        {
            ReorderableList reordList = new ReorderableList(serializedObject,
                serializedObject.FindProperty("m_tilemaps"), true, true, true, true);
            ...
            reordList.drawHeaderCallback = (Rect rect) =>
            {
                ...
                if (GUI.Button(new Rect(rect.x + rect.width - rect.height, rect.y, rect.height, rect.height),
                    btnTexture, EditorStyles.label))
                {
                    GUI.changed = true;
            ...
            reordList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                if (tilemap)
                {
                    ...
                    {
                        tilemapTilemapObjectSerialized[tilemapSerialized] = tilemapObjSerialized =
                            new SerializedObject(tilemapSerialized.FindProperty("m_GameObject").objectReferenceValue);
                    }

                    Rect rToggle = new Rect(rect.x, rect.y, 16f, EditorGUIUtility.singleLineHeight);
                    Rect rName = new Rect(rect.x + 20f, rect.y, rect.width - 130f - 20f,
                        EditorGUIUtility.singleLineHeight);
                    Rect rColliders = new Rect(rect.x + rect.width - 125f, rect.y, 125f,
                        EditorGUIUtility.singleLineHeight);
                    Rect rSortingLayer = new Rect(rect.x + rect.width - 125f, rect.y, 80f,
                        EditorGUIUtility.singleLineHeight);
                    Rect rSortingOrder = new Rect(rect.x + rect.width - 40f, rect.y, 40f,
                        EditorGUIUtility.singleLineHeight);

                    tilemap.IsVisible = EditorGUI.Toggle(rToggle, GUIContent.none, tilemap.IsVisible,
                        STEditorStyles.Instance.visibleToggleStyle);
                    ...
                    if (TilemapEditor.EditMode == TilemapEditor.eEditMode.Collider)
                    {
                        ...
                        string[] colliderTypeNames =
                            new List<string>(System.Enum.GetNames(typeof(eColliderType))
                                .Select(x => x.Replace('_', ' '))).ToArray();
                        EditorGUI.BeginChangeCheck();
                        colliderTypeProperty.intValue =
                            GUI.Toolbar(rColliders, colliderTypeProperty.intValue, colliderTypeNames);
                    ...
                    else
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        EditorGUI.PropertyField(rSortingLayer, tilemapSerialized.FindProperty("m_sortingLayer"),
                            GUIContent.none);
                        EditorGUI.PropertyField(rSortingOrder, tilemapSerialized.FindProperty("m_orderInLayer"),
                            GUIContent.none);
                        tilemapSerialized.FindProperty("m_orderInLayer").intValue =
                            (tilemapSerialized.FindProperty("m_orderInLayer").intValue << 16) >>
                            16; // convert from int32 to int16 keeping sign
                        if (EditorGUI.EndChangeCheck())
                        {
                            tilemapSerialized.ApplyModifiedProperties();
