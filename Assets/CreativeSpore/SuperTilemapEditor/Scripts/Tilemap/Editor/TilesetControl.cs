    ...
    public class TilesetControl
    {
        private class Styles
        {
            ...
            public static Styles Instance
            {
                get
                {
                    if (s_instance == null) s_instance = new Styles();
                    return s_instance;
                }
            }
        ...
        private Dictionary<Tileset, SharedTilesetData> m_dicTilesetSharedData =
            new Dictionary<Tileset, SharedTilesetData>();
        ...
        public bool Display()
        {
            ...
            AssetPreview
                .SetPreviewTextureCacheSize(
                    256); //FIX clickeing issues when displaying multiple prefab previews (when a tile has a prefab attached
            ...
            if (m_sharedData.tileViewList == null || m_sharedData.tileViewList.list != Tileset.TileViews)
            {
                ...
                m_sharedData.tileViewList.onRemoveCallback += (ReorderableList list) => { RemoveTileSelection(); };
            }
            ...
            // Draw TileView ReorderableList
            {
                ...
                m_sharedData.tileViewList.index =
                    Mathf.Clamp(m_sharedData.tileViewList.index, -1, Tileset.TileViews.Count - 1);
            ...
            TileView tileView = m_sharedData.tileViewList != null && m_sharedData.tileViewList.index >= 0
                ? Tileset.TileViews[m_sharedData.tileViewList.index]
                : null;
            ...
            if (tileView == null)
            {
                ...
                Tileset.TileRowLength = Mathf.Clamp(EditorGUILayout.IntField("TileRowLength", Tileset.TileRowLength), 1,
                    Tileset.Tiles.Count);
            ...
            EditorGUI.BeginChangeCheck();
            m_sharedData.tileViewList.index = EditorGUILayout.IntPopup("Tileset View", m_sharedData.tileViewList.index,
                tileViewNames, tileViewValues);
            if (EditorGUI.EndChangeCheck())
            {
                RemoveTileSelection();
            }
            ...
            if (m_prevBgColor != Tileset.BackgroundColor || Styles.Instance.scrollStyle.normal.background == null)
            {
                ...
                if (Styles.Instance.scrollStyle.normal.background == null)
                    Styles.Instance.scrollStyle.normal.background =
                        new Texture2D(1, 1) {hideFlags = HideFlags.DontSave};
            ...
            string sTileIdLabel = Tileset.SelectedTileId != Tileset.k_TileId_Empty
                ? " (id:" + Tileset.SelectedTileId + ")"
                : "";
            EditorGUILayout.LabelField("Tile Palette" + sTileIdLabel, EditorStyles.boldLabel);

            m_sharedData.tileViewRowLength =
                tileView != null ? tileView.tileSelection.rowLength : Tileset.TileRowLength;
            // keeps values safe
            ...
            float tileAreaHeight = (Tileset.VisualTileSize.y + visualTilePadding) *
                                   (1 + (m_visibleTileCount - 1) / m_sharedData.tileViewRowLength) + 4f;
            ...
            Vector2 tilesScrollPos = EditorGUILayout.BeginScrollView(m_sharedData.tilesScrollPos,
                Styles.Instance.scrollStyle /*, GUILayout.MinHeight(minTileScrollHeight)*/);
            if (m_updateScrollPos) m_sharedData.tilesScrollPos = tilesScrollPos;
            {
                ...
                if (Tileset.Tiles != null)
                {
                    ...
                    int tileViewHeight = tileView != null
                        ? ((tileView.tileSelection.selectionData.Count - 1) / tileView.tileSelection.rowLength) + 1
                        : Tileset.Height;
                    int totalCount =
                        ((((tileViewWidth - 1) / m_sharedData.tileViewRowLength) + 1) *
                         m_sharedData.tileViewRowLength) * tileViewHeight;
                    int tileIdOffset = 0;
                    for (int i = 0; i < totalCount; ++i)
                    {
                        int tileId =
                            GetTileIdFromIdx(i, m_sharedData.tileViewRowLength, tileViewWidth, tileViewHeight) +
                            tileIdOffset;
                        ...
                        Rect rVisualTile = new Rect(2 + tx * (Tileset.VisualTileSize.x + visualTilePadding),
                            2 + ty * (Tileset.VisualTileSize.y + visualTilePadding), Tileset.VisualTileSize.x,
                            Tileset.VisualTileSize.y);

                        // Optimization, skipping not visible tiles
                        Rect rLocalVisualTile = rVisualTile;
                        rLocalVisualTile.position -= m_sharedData.tilesScrollPos;
                            ...
                            //---
                        {
                            ...
                            if (tile == null)
                            {
                                HandlesEx.DrawRectWithOutline(rVisualTile, new Color(0f, 0f, 0f, 0.2f),
                                    new Color(0f, 0f, 0f, 0.2f));
                            }
                            else
                            {
                                HandlesEx.DrawRectWithOutline(rVisualTile, new Color(0f, 0f, 0f, 0.1f),
                                    new Color(0f, 0f, 0f, 0.1f));
                                TilesetEditor.DoGUIDrawTileFromTileData(rVisualTile, tileData, Tileset);
                            }

                            Rect rTileRect = new Rect(2 + tx * (Tileset.VisualTileSize.x + visualTilePadding),
                                2 + ty * (Tileset.VisualTileSize.y + visualTilePadding),
                                (Tileset.VisualTileSize.x + visualTilePadding),
                                (Tileset.VisualTileSize.y + visualTilePadding));
                            if (rVisualTile.Contains(e.mousePosition))
                            {
                                if (e.type == EventType.MouseDrag && e.button == 0)
                                    m_sharedData.pointedTileIdxRect =
                                        new KeyValuePair<int, Rect>(m_visibleTileCount, rTileRect);
                                else if (e.type == EventType.MouseDown && e.button == 0)
                                    m_sharedData.startDragTileIdxRect = m_sharedData.pointedTileIdxRect =
                                        m_sharedData.endDragTileIdxRect =
                                            new KeyValuePair<int, Rect>(m_visibleTileCount, rTileRect);
                            ...
                            if ((isLeftMouseReleased || isRightMouseReleased) && isInsideTileScrollArea &&
                                rVisualTile.Contains(e.mousePosition)
                                && (m_sharedData.startDragTileIdxRect.Key ==
                                    m_sharedData.endDragTileIdxRect.Key) // and there is not dragging selection
                                && m_rTileScrollSize.Contains(e.mousePosition - m_sharedData.tilesScrollPos)
                            ) // and it's inside the scroll area
                            {
                            ...
                            {
                                HandlesEx.DrawRectWithOutline(rTileRect, new Color(0f, 0f, 0f, 0.1f),
                    ...
                    // Draw selection rect
                    if (m_sharedData.startDragTileIdxRect.Key !=
                        m_sharedData.pointedTileIdxRect
                            .Key /*&& m_startDragTileIdxRect.Key == m_endDragTileIdxRect.Key*/)
                    {
                        Rect rSelection = new Rect(m_sharedData.startDragTileIdxRect.Value.center,
                            m_sharedData.pointedTileIdxRect.Value.center -
                            m_sharedData.startDragTileIdxRect.Value.center);
                        rSelection.Set(Mathf.Min(rSelection.xMin, rSelection.xMax),
                            Mathf.Min(rSelection.yMin, rSelection.yMax), Mathf.Abs(rSelection.width),
                            Mathf.Abs(rSelection.height));
                        ...
                        HandlesEx.DrawRectWithOutline(rSelection, new Color(0f, 0f, 0f, 0.1f),
            ...
            if (e.type == EventType.Repaint)
            {
                ...
                m_rTileScrollSize.position =
                    Vector2.zero; // reset position to the Contains and Overlaps inside the tile scroll view without repositioning the position of local positions
            ...
            float fBrushesScrollHeight = Mathf.Min(fBrushesScrollMaxHeight,
                4 + (Tileset.VisualTileSize.y + visualTilePadding) * (1 + (Tileset.Brushes.Count / tileRowLength)));
            ...
            else
            {
                ...
                Vector2 brushesScrollPos =
                    EditorGUILayout.BeginScrollView(m_sharedData.brushesScrollPos, Styles.Instance.scrollStyle);
                if (m_updateScrollPos) m_sharedData.brushesScrollPos = brushesScrollPos;
                {
                    ...
                    tileRowLength =
                        Mathf.Clamp((int) rScrollView.width / (int) (Tileset.VisualTileSize.x + visualTilePadding), 1,
                            tileRowLength);
                    if (Tileset.Brushes != null)
                    {
                        GUILayout.Space((Tileset.VisualTileSize.y + visualTilePadding) *
                                        (1 + (Tileset.Brushes.Count - 1) / tileRowLength));
                        for (int i = 0, idx = 0; i < Tileset.Brushes.Count; ++i, ++idx)
                        {
                            ...
                            if (!brushCont.BrushAsset.ShowInPalette ||
                                !Tileset.IsBrushVisibleByTypeMask(brushCont.BrushAsset))
                            {
                                --idx;
                                continue;
                            }
                            ...
                            {
                                Debug.LogWarning("Fixed missing tileset reference in brush " +
                                                 brushCont.BrushAsset.name + " Id(" + brushCont.Id + ")");
                            ...
                            TilesetEditor.DoGUIDrawTileFromTileData(rVisualTile, tileData, Tileset,
                                brushCont.BrushAsset.GetAnimUV());
                            if ((isLeftMouseReleased || isRightMouseReleased || m_dblClick.IsDblClick) &&
                                isInsideBrushScrollArea && rVisualTile.Contains(Event.current.mousePosition))
                            {
                            ...
                            {
                                Rect rSelection = new Rect(2 + tx * (Tileset.VisualTileSize.x + visualTilePadding),
                                    2 + ty * (Tileset.VisualTileSize.y + visualTilePadding),
                                    (Tileset.VisualTileSize.x + visualTilePadding),
                                    (Tileset.VisualTileSize.y + visualTilePadding));
                                HandlesEx.DrawRectWithOutline(rSelection, new Color(0f, 0f, 0f, 0.1f),
        ...
        private void DoAutoScroll()
        {
            ...
            if (m_rTileScrollSize.Contains(e.mousePosition - m_sharedData.tilesScrollPos))
            {
                if (e.type == EventType.MouseDrag && e.button == 0)
                {
                    ...
                    float leftFactorX = mouseMoveDisp.x < 0f
                        ? 1f - Mathf.Pow(Mathf.Clamp01(mousePosition.x / autoScrollDist), 2)
                        : 0f;
                    float rightFactorX = mouseMoveDisp.x > 0f
                        ? 1f - Mathf.Pow(Mathf.Clamp01((m_rTileScrollSize.width - mousePosition.x) / autoScrollDist), 2)
                        : 0f;
                    float topFactorY = mouseMoveDisp.y < 0f
                        ? 1f - Mathf.Pow(Mathf.Clamp01(mousePosition.y / autoScrollDist), 2)
                        : 0f;
                    float bottomFactorY = mouseMoveDisp.y > 0f
                        ? 1f - Mathf.Pow(Mathf.Clamp01((m_rTileScrollSize.height - mousePosition.y) / autoScrollDist),
                              2)
                        : 0f;
                    m_tileScrollSpeed = autoScrollSpeed *
                                        new Vector2((-leftFactorX + rightFactorX), (-topFactorY + bottomFactorY));
                }
        ...
        private void DoSetTileSelection()
        {
            ...
            {
                int tx_start = Mathf.Min(m_sharedData.startDragTileIdxRect.Key % m_sharedData.tileViewRowLength,
                    m_sharedData.endDragTileIdxRect.Key % m_sharedData.tileViewRowLength);
                int ty_start = Mathf.Min(m_sharedData.startDragTileIdxRect.Key / m_sharedData.tileViewRowLength,
                    m_sharedData.endDragTileIdxRect.Key / m_sharedData.tileViewRowLength);
                int tx_end = Mathf.Max(m_sharedData.startDragTileIdxRect.Key % m_sharedData.tileViewRowLength,
                    m_sharedData.endDragTileIdxRect.Key % m_sharedData.tileViewRowLength);
                int ty_end = Mathf.Max(m_sharedData.startDragTileIdxRect.Key / m_sharedData.tileViewRowLength,
                    m_sharedData.endDragTileIdxRect.Key / m_sharedData.tileViewRowLength);
                List<uint> selectionData = new List<uint>();
                int tileIdx = 0;
                for (int ty = ty_end;
                    ty >= ty_start;
                    --ty) // NOTE: this goes from bottom to top to fit the tilemap coordinate system
                {
