...
namespace CreativeSpore.SuperTilemapEditor
{
    public class BrushTileGridControl
    {
        ...
        public void Display(Object target, uint[] aTileData, int[] tileIdxMap, int gridWidth, int gridHeight,
            Vector2 visualTileSize, int[] symbolIdxMap)
        {
            ...
            GUILayout.BeginScrollView(Vector2.zero, (GUIStyle) "Button", GUILayout.Width(visualTileSize.x * gridWidth),
                GUILayout.Height(visualTileSize.y * gridHeight + 1f));
            ...
            for (int i = 0; i < gridWidth * gridHeight; ++i)
            {
                ...
                Rect rVisualTile = new Rect(gx * visualTileSize.x, gy * visualTileSize.y, visualTileSize.x,
                    visualTileSize.y);
                ...
                {
                    GUI.DrawTexture(rVisualTile, GetTileSymbolTexture((byte) symbolIdxMap[i]), ScaleMode.ScaleToFit,
                        true);
                }

                Color bgColor = new Color(1f - Tileset.BackgroundColor.r, 1f - Tileset.BackgroundColor.g,
                    1f - Tileset.BackgroundColor.b, Tileset.BackgroundColor.a);
                HandlesEx.DrawRectWithOutline(rVisualTile,
                    m_selectedTileIdx == tileIdx ? new Color(0f, 0f, 0f, 0.1f) : new Color(),
                    m_selectedTileIdx == tileIdx ? new Color(1f, 1f, 0f, 1f) : bgColor);
            ...
            uint brushTileData =
                m_selectedTileIdx >= 0 ? m_aBrushTileData[m_selectedTileIdx] : Tileset.k_TileData_Empty;
            ...
            m_displayAutocompleteBtn = e.type == EventType.Layout
                ? !hasEmptyTiles && m_tileIdOff != 0
                : m_displayAutocompleteBtn;
            ...
            if (Tileset.TileSelection != null && Tileset.TileSelection.selectionData.Count == gridWidth * gridHeight &&
                Tileset.TileSelection.rowLength == gridWidth)
            {
                if (GUILayout.Button("Autocomplete from selection"))
                {
                    ...
                    for (int i = 0; i < tileIdxMap.Length; ++i)
                    {
                        ...
                        int brushTileId = (int) (Tileset.TileSelection.selectionData[selectionIdx] &
                                                 Tileset.k_TileDataMask_TileId);
                        m_aBrushTileData[tileIdx] = (uint) (brushTileId & Tileset.k_TileDataMask_TileId);
                    }
            ...
            if (ShowHelpBox)
            {
                EditorGUILayout.HelpBox(
                    "Select  a tile from the grid, then select a tile from Tile Selection Window to change the tile.\nOr select a group of tiles and press Autocomplete from selection.",
                    MessageType.Info);
            }
        }
        ...
        public static uint DoTileDataPropertiesLayout(uint tileData, Tileset tileset, bool displayBrush = true)
        {
            ...
            if (displayBrush)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    {
                        Debug.LogWarning("The brush " + brush.name +
                                         " belongs to a different tileset and cannot be selected! ");
                    }
                    else
                    {
                        brushId = brush != null ? tileset.FindBrushId(brush.name) : Tileset.k_BrushId_Default;
                        int tileId = brush != null
                            ? (int) (brush.PreviewTileData() & Tileset.k_TileDataMask_TileId)
                            : Tileset.GetTileIdFromTileData(tileData);
