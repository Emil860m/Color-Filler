...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilePropertiesControl
    {
        ...
        [SerializeField] private eEditMode m_editMode;
        ...
        public void Display()
        {
            ...
            if (Tileset.SelectedTileId == Tileset.k_TileId_Empty && Tileset.TileSelection == null &&
                Tileset.SelectedBrushId == Tileset.k_BrushId_Default)
            {
            ...
            m_scrollPos =
                EditorGUILayout.BeginScrollView(m_scrollPos, GUILayout.Width(EditorGUIUtility.currentViewWidth));
            {
                ...
                switch (m_editMode)
                {
                    case eEditMode.Collider:
                        DisplayCollider();
                        break;
                    case eEditMode.Parameters:
                        DisplayParameters();
                        break;
                    case eEditMode.Prefab:
                        DisplayPrefab();
                        break;
                    case eEditMode.Autotiling:
                        DisplayAutotiling();
                        break;
                }
            }
        ...
        private void DisplayPrefab()
        {
            ...
            else
            {
                ...
                Tile selectedTile = isMultiselection
                    ? Tileset.Tiles[(int) (Tileset.TileSelection.selectionData[0] & Tileset.k_TileDataMask_TileId)]
                    : Tileset.SelectedTile;
                GUILayoutUtility.GetRect(1, 1, GUILayout.Width(Tileset.VisualTileSize.x),
                    GUILayout.Height(Tileset.VisualTileSize.y));
                Rect tileUV = selectedTile.uv;
                GUI.color = Tileset.BackgroundColor;
                GUI.DrawTextureWithTexCoords(GUILayoutUtility.GetLastRect(), EditorGUIUtility.whiteTexture, tileUV,
                    true);
                ...
                prefabData.offsetMode =
                    (TilePrefabData.eOffsetMode) EditorGUILayout.EnumPopup("Offset Mode", prefabData.offsetMode);
                prefabData.rotation = EditorGUILayout.Vector3Field("Rotation", prefabData.rotation);
                EditorGUI.BeginChangeCheck();
                GameObject prevPrefab = prefabData.prefab;
                prefabData.prefab =
                    (GameObject) EditorGUILayout.ObjectField("Prefab", prefabData.prefab, typeof(GameObject), false);
                bool isPrefabChanged = EditorGUI.EndChangeCheck();
                // Special case for 3D tiles where tilemap will be rotated over the plane XZ
                if (isPrefabChanged && !prevPrefab && prefabData.rotation == Vector3.zero && prefabData.prefab &&
                    prefabData.prefab.GetComponentInChildren<MeshRenderer>())
                {
                ...
                if (prefabData.prefab)
                {
                    ...
                    if (prefabPreview)
                        prefabData.showPrefabPreviewInTilePalette = EditorGUILayout.Toggle(
                            "Display the prefab preview in the tile palette",
                            prefabData.showPrefabPreviewInTilePalette);
                    EditorGUIUtility.labelWidth = savedLabelWidth;
                    prefabData.showTileWithPrefab =
                        EditorGUILayout.Toggle("Show Tile With Prefab", prefabData.showTileWithPrefab);
                }
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (isMultiselection)
                    {
                        ...
                        {
                            Tile tile = Tileset.Tiles[
                                (int) (Tileset.TileSelection.selectionData[i] & Tileset.k_TileDataMask_TileId)];
        ...
        private void DisplayAutotiling()
        {
            if (Tileset.SelectedBrushId != Tileset.k_BrushId_Default)
            {
                ...
                if (brush)
                {
                    ...
                    GUILayoutUtility.GetRect(1, 1, GUILayout.Width(Tileset.VisualTileSize.x),
                        GUILayout.Height(Tileset.VisualTileSize.y));
                    TilesetEditor.DoGUIDrawTileFromTileData(GUILayoutUtility.GetLastRect(), brush.GetAnimTileData(),
                        Tileset, brush.GetAnimUV());
            ...
            else
            {
                ...
                selectedTile.autilingGroup =
                    TilesetEditor.DoGroupFieldLayout(Tileset, "Group", selectedTile.autilingGroup);
        ...
        private void DisplayParameters()
        {
            ...
            if (tileSelection != null)
            {
                ...
                EditorGUILayout.HelpBox(
                    "This will combine all the parameters found in the tile selection so if a parameter is missing in a tile, it will add the parameter, but if it has it already, it won't change the value.",
                    MessageType.Info);
            ...
            else
            {
                ...
                {
                    if (prevOwner != m_parameterListOwner || Tileset.SelectedTileId != m_prevSelectedTileId ||
                        m_tileParameterList == null)
                    {
                    ...
                    Rect customUV = m_parameterListOwner is Tileset
                        ? default(Rect)
                        : ((TilesetBrush) m_parameterListOwner).GetAnimUV();
                    GUI.color = Tileset.BackgroundColor;
                    GUI.DrawTexture(GUILayoutUtility.GetLastRect(), EditorGUIUtility.whiteTexture);
                    GUI.color = Color.white;
                    TilesetEditor.DoGUIDrawTileFromTileData(GUILayoutUtility.GetLastRect(), tilePreviewData, Tileset,
                        customUV);

                    m_tileParamScrollViewPos = EditorGUILayout.BeginScrollView(m_tileParamScrollViewPos,
        ...
        private void DisplayCollider()
        {
            ...
            if (m_prevBgColor != Tileset.BackgroundColor || Styles.Instance.colliderBgStyle.normal.background == null)
            {
                ...
                if (Styles.Instance.colliderBgStyle.normal.background == null)
                    Styles.Instance.colliderBgStyle.normal.background =
            ...
            if (e.type == EventType.Repaint)
            {
                ...
                m_rTile = new Rect(padding * pixelSize, padding * pixelSize, rCollArea.width - 2 * padding * pixelSize,
                    (rCollArea.width / aspectRatio) - 2 * padding * pixelSize);
            ...
            {
                Vector2[] collVertices = selectedTile.collData.type == eTileCollider.Full
                    ? s_fullCollTileVertices
                    : selectedTile.collData.vertices;
                ...
                for (int i = 0; i < collVertices.Length; ++i)
                {
                    ...
                    s1.x = m_rTile.x + m_rTile.width * s1.x;
                    s1.y = m_rTile.yMax - m_rTile.height * s1.y;
                    ...
                    //Draw normals
                    {
                        ...
                        Handles.DrawLine(normPos,
                            normPos + Vector3.Cross(s1 - s0, Vector3.forward).normalized * m_rTile.yMin);
                    }
                ...
                if (selectedTile.collData.type == eTileCollider.Polygon)
                {
                    ...
                    bool isRemovingVertexOn = !m_isDragging &&
                                              ((Application.platform == RuntimePlatform.OSXEditor)
                                                  ? e.command
                                                  : e.control) && collVertices.Length > 3;
                    ...
                    for (int i = 0; i < collVertices.Length; ++i)
                    {
                        ...
                        if (e.type == EventType.Repaint)
                        {
                            if (i == m_activeVertexIdx)
                            {
                                ...
                                GUI.Label(new Rect(0, 0, m_rTile.width, m_rTile.yMin),
                                    Vector2.Scale(collVertices[i], Tileset.TilePxSize).ToString(),
                                    Styles.Instance.vertexCoordStyle);
                            }

                            GUI.color = m_activeVertexIdx == i
                                ? (isRemovingVertexOn ? Color.red : Color.cyan)
                                : new Color(0.7f, 0.7f, 0.7f, 0.8f);
                            Styles.Instance.collVertexHandleStyle.Draw(
                                new Rect(s0.x - Styles.Instance.collVertexHandleStyle.normal.background.width / 2,
                                    s0.y - Styles.Instance.collVertexHandleStyle.normal.background.height / 2, 1, 1),
                                i.ToString(), false, false, false, false);
                            GUI.color = Color.white;
                        }
                    }
                    ...
                    if (isAddingVertexOn)
                    {
                        ...
                        if (e.type == EventType.MouseUp)
                        {
                            ...
                            for (int i = 0, j = 0; i < selectedTile.collData.vertices.Length; ++i)
                            {
                                if (segmentIdx == i)
                                {
                                    newVertexPos.x = Mathf.Clamp(Mathf.Round(newVertexPos.x / pixelSize) * pixelSize,
                                        m_rTile.x, m_rTile.xMax);
                                    newVertexPos.y = Mathf.Clamp(Mathf.Round(newVertexPos.y / pixelSize) * pixelSize,
                                        m_rTile.y, m_rTile.yMax);
                                    selectedTile.collData.vertices[i].x =
                                        Mathf.Clamp01((newVertexPos.x - m_rTile.x) / m_rTile.width);
                                    selectedTile.collData.vertices[i].y =
                                        Mathf.Clamp01((newVertexPos.y - m_rTile.yMax) / -m_rTile.height);
                                }
                                else
                                {
                        ...
                        else if (e.type == EventType.Repaint)
                        {
                            ...
                            Styles.Instance.collVertexHandleStyle.Draw(
                                new Rect(
                                    newVertexPos.x - Styles.Instance.collVertexHandleStyle.normal.background.width / 2,
                                    newVertexPos.y - Styles.Instance.collVertexHandleStyle.normal.background.height / 2,
                                    1, 1), segmentIdx.ToString(), false, false, false, false);
                            GUI.color = Color.white;
                        }
                    }
                }
            ...
            string helpInfo =
                ...
                " + Click for removing a vertex. (should be more than 3)" + "\n" +
            ...
            selectedTile.collData.type =
                (eTileCollider) GUILayout.Toolbar((int) selectedTile.collData.type, tileColliderNames);
            EditorGUI.indentLevel -= 2;
            ...
            if (saveChanges)
            {
                if (isMultiselection)
                {
                    for (int i = 0; i < Tileset.TileSelection.selectionData.Count; ++i)
                    {
                        Tileset.Tiles[(int) (Tileset.TileSelection.selectionData[i] & Tileset.k_TileDataMask_TileId)]
                            .collData = selectedTile.collData.Clone();
                    }
                }

                EditorUtility.SetDirty(Tileset);
                //Refresh selected tilemap
                STETilemap selectedTilemap = Selection.activeGameObject
                    ? Selection.activeGameObject.GetComponent<STETilemap>()
                    : null;
                if (selectedTilemap)
        ...
        public static ReorderableList CreateParameterReorderableList(ParameterContainer paramContainer)
        {
            ReorderableList reordList =
                new ReorderableList(paramContainer.ParameterList, typeof(Parameter), true, true, true, true);
            reordList.onAddDropdownCallback = (Rect buttonRect, ReorderableList l) =>
            {
                ...
                {
                    if (EditorUtility.DisplayDialog("Warning!", "Are you sure you want to delete all items?", "Yes",
                        "No"))
                    {
            ...
            reordList.drawHeaderCallback = (Rect rect) =>
            {
                ...
                Texture2D btnTexture = reordList.elementHeight == 0f
                    ? EditorGUIUtility.FindTexture("winbtn_win_max_h")
                    : EditorGUIUtility.FindTexture("winbtn_win_min_h");
                if (GUI.Button(new Rect(rect.width - rect.height, rect.y, rect.height, rect.height), btnTexture,
                    EditorStyles.label))
                {
            ...
            reordList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                if (index == 0)
                {
                    ...
                    {
                        s_maxLabelNameSize = Mathf.Max(s_maxLabelNameSize,
                            EditorStyles.label.CalcSize(new GUIContent(p.name + "."))
                                .x); //NOTE: adding '.' to make sure ending spaces are not skipped
                    }
                }
                ...
                Rect rParamValue = new Rect(rLabel.x + s_maxLabelNameSize, rLabel.y, rect.width - s_maxLabelNameSize,
                    rLabel.height);
                ...
                {
                    param.SetValue(EditorGUI.ObjectField(rParamValue, param.GetAsObject(), typeof(UnityEngine.Object),
                        false));
                }
            };
