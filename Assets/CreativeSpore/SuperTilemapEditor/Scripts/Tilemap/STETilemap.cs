...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class STETilemap : MonoBehaviour
    {
        ...
        {
            get { return m_physicMaterial2D; }
            set { m_physicMaterial2D = value; }
        }
        ...
        {
            get { return m_allowPaintingOutOfBounds; }
            set { m_allowPaintingOutOfBounds = value; }
        }
        ...
        {
            get { return m_autoShrink; }
            set { m_autoShrink = value; }
        }
        ...
        {
            get { return m_enableUndoWhilePainting; }
            set { m_enableUndoWhilePainting = value; }
        }
        ...
        public int MinGridX
        {
            get { return m_minGridX; } /*set { m_minGridX = Mathf.Min(0, value); }*/
        }
        ...
        public int MinGridY
        {
            get { return m_minGridY; } /*set { m_minGridY = Mathf.Min(0, value); }*/
        }
        ...
        public int MaxGridX
        {
            get { return m_maxGridX; } /*set { m_maxGridX = Mathf.Max(0, value); }*/
        }
        ...
        public int MaxGridY
        {
            get { return m_maxGridY; } /*set { m_maxGridY = Mathf.Max(0, value); }*/
        }
        ...
        {
            get { return m_maxGridX - m_minGridX + 1; }
        }
        ...
        {
            get { return m_maxGridY - m_minGridY + 1; }
        }
        ...
        {
            get { return m_parentTilemapGroup; }
        }
        ...
        public ChunkRendererPropertiesData ChunkRendererProperties
        {
            get { return m_chunkRendererProperties; }
        }
        ...
        [SerializeField] private bool m_allowPaintingOutOfBounds = true;
        [SerializeField] private bool m_autoShrink = false;
        ...
        [SerializeField] private PhysicsMaterial2D m_physicMaterial2D;

        [SerializeField] Vector2 m_cellSize;
        [SerializeField] private Bounds m_mapBounds;
        [SerializeField] private Tileset m_tileset;
        [SerializeField] private int m_minGridX;
        [SerializeField] private int m_minGridY;
        [SerializeField] private int m_maxGridX;
        [SerializeField] private int m_maxGridY;
        [SerializeField] private TilemapGroup m_parentTilemapGroup;
        [SerializeField] private Vector2 m_parallaxFactor = Vector2.one;
        ...
        private void _OnPreCull(Camera cam)
        {
            ...
            transform.localPosition = m_preRenderPos +
                                      (Vector3) (Vector2.Scale(cam.transform.position,
                                          (Vector2.one - m_parallaxFactor)));
        }
        ...
        public void OnDrawGizmos()
        {
            if (ColliderDisplayMode == eCollDisplayMode.Always ||
                (ColliderDisplayMode == eCollDisplayMode.ParentSelected && Selection.activeGameObject &&
                 this.gameObject.transform.IsChildOf(Selection.activeGameObject.transform))
            )
            {
                DoDrawGizmos();
            }
        ...
        void DoDrawGizmos()
        {
            ...
            transform.localPosition +=
                (Vector3) (Vector2.Scale(Camera.current.transform.position,
                    (Vector2.one - m_parallaxFactor))); //apply parallax
            ...
            if (ShowGrid)
            {
                ...
                if (HandleUtility.GetHandleSize(rayCamToPlane.GetPoint(distCamToTilemap)) <= 3f)
                {
                    ...
                    for (float i = 1; i < GridWidth; i++)
                    {
                        Gizmos.DrawLine(
                            this.transform.TransformPoint(
                                new Vector3(MapBounds.min.x + i * CellSize.x, MapBounds.min.y)),
                            this.transform.TransformPoint(
                                new Vector3(MapBounds.min.x + i * CellSize.x, MapBounds.max.y))
                        );
                    }
                    ...
                    {
                        Gizmos.DrawLine(
                            this.transform.TransformPoint(new Vector3(MapBounds.min.x, MapBounds.min.y + i * CellSize.y,
                                0)),
                            this.transform.TransformPoint(new Vector3(MapBounds.max.x, MapBounds.min.y + i * CellSize.y,
                                0))
                        );
                    }
                }
            ...
            while (valueIter.MoveNext())
            {
                ...
                if (chunk)
                {
                    ...
                    rBound = new Rect(chunkX * k_chunkSize * CellSize.x, chunkY * k_chunkSize * CellSize.y,
                        k_chunkSize * CellSize.x, k_chunkSize * CellSize.y);
        ...
        public void Refresh(bool refreshMesh = true, bool refreshMeshCollider = true, bool refreshTileObjects = false,
            bool invalidateBrushes = false)
        {
            BuildTilechunkDictionary();
            var valueIter = m_dicChunkCache.Values.GetEnumerator();
            while (valueIter.MoveNext())
        ...
        public void SetTileColor(Vector2 vLocalPos, TileColor32 tileColor,
            eBlendMode blendMode = eBlendMode.AlphaBlending)
        {
        ...
        /// <param name="c2">Top left corner</param>
        /// <param name="c3">Top right corner</param>
        public void SetTileColor(int gridX, int gridY, TileColor32 tileColor,
            eBlendMode blendMode = eBlendMode.AlphaBlending)
        {
            ...
            if (m_allowPaintingOutOfBounds ||
                (gridX >= m_minGridX && gridX <= m_maxGridX && gridY >= m_minGridY && gridY <= m_maxGridY))
            {
        ...
        public void SetTile(Vector2 vLocalPos, int tileId, int brushId = Tileset.k_BrushId_Default,
            eTileFlags flags = eTileFlags.None)
        {
            int gridX = BrushUtil.GetGridX(vLocalPos, CellSize);
            int gridY = BrushUtil.GetGridY(vLocalPos, CellSize);
        ...
        /// <param name="brushId"></param>
        /// <param name="flags"></param>
        public void SetTile(int gridX, int gridY, int tileId, int brushId = Tileset.k_BrushId_Default,
            eTileFlags flags = eTileFlags.None)
        {
            uint tileData = ((uint) flags << 28) | (((uint) brushId << 16) & Tileset.k_TileDataMask_BrushId) |
            ...
            for (int gy = MinGridY; gy <= MaxGridY; ++gy)
            {
                for (int gx = MinGridX; gx <= MaxGridX; ++gx, ++idx)
                {
                    ...
                    if (
                        ...
                        && (flippedTileData & Tileset.k_TileDataMask_BrushId) ==
                        0 // don't activate flip flags on brushes
                    )
                    {
                        flippedTileData =
                            TilesetBrush.ApplyAndMergeTileFlags(flippedTileData, Tileset.k_TileFlag_FlipV);
                    }
        ...
        public void FlipH(bool changeFlags)
        {
            ...
            for (int gx = MinGridX; gx <= MaxGridX; ++gx)
            {
                for (int gy = MinGridY; gy <= MaxGridY; ++gy, ++idx)
                {
                    ...
                    {
                        flippedTileData =
                            TilesetBrush.ApplyAndMergeTileFlags(flippedTileData, Tileset.k_TileFlag_FlipH);
                    }
        ...
        public void Rot90(bool changeFlags)
        {
            ...
            for (int gx = minGridX; gx <= maxGridX; ++gx)
            {
                for (int gy = maxGridY; gy >= minGridY; --gy, ++idx)
                {
                    ...
                    {
                        flippedTileData =
                            TilesetBrush.ApplyAndMergeTileFlags(flippedTileData, Tileset.k_TileFlag_Rot90);
                    }
        ...
        public bool InvalidateChunkAt(int gridX, int gridY, bool invalidateMesh = true,
            bool invalidateMeshCollider = true)
        {
            TilemapChunk chunk = GetOrCreateTileChunk(gridX, gridY);
            if (chunk != null)
            {
        ...
        private TilemapChunk GetOrCreateTileChunk(int gridX, int gridY, bool createIfDoesntExist = false)
        {
            ...
            if (tilemapChunk == null && createIfDoesntExist)
            {
                ...
                tilemapChunk =
                    chunkObj
                        .AddComponent<TilemapChunk
                        >(); //NOTE: this call TilemapChunk.OnEnable before initializing the TilemapChunk. Make all changes after this.
                chunkObj.transform.parent = transform;
                chunkObj.transform.localPosition =
                    new Vector2(chunkX * k_chunkSize * CellSize.x, chunkY * k_chunkSize * CellSize.y);
                chunkObj.transform.localRotation = Quaternion.identity;
                chunkObj.transform.localScale = Vector3.one;
                chunkObj.hideFlags =
                    gameObject.hideFlags |
                    HideFlags
                        .HideInHierarchy; //NOTE: note the flags inheritance. BrushBehaviour object is not saved, so chunks are left orphans unless this inheritance is done
        ...
        private void BuildTilechunkDictionary()
        {
            ...
            for (int i = 0; i < transform.childCount; ++i)
            {
                ...
                if (chunk)
                {
                    int chunkX = (chunk.GridPosX < 0 ? (chunk.GridPosX + 1 - k_chunkSize) : chunk.GridPosX) /
                                 k_chunkSize;
                    int chunkY = (chunk.GridPosY < 0 ? (chunk.GridPosY + 1 - k_chunkSize) : chunk.GridPosY) /
                                 k_chunkSize;
                    uint key = (uint) ((chunkY << 16) | (chunkX & 0x0000FFFF));
