...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class AnimBrushEditor : TilesetBrushEditor
    {
        ...
        public override void OnEnable()
        {
            ...
            m_frameList = new ReorderableList(serializedObject, serializedObject.FindProperty("AnimFrames"), true, true,
                true, true);
            ...
            m_frameList.drawElementCallback += (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                Vector2 UvPxOffset = new Vector2(uvOffset.x / m_brush.Tileset.AtlasTexture.texelSize.x,
                ...
                for (int gy = 0; gy < 2; ++gy)
                {
                    ...
                    {
                        Rect btnRect = new Rect(rect.x + rect.width - (gx + 1) * btnSize,
                            rect.y + gy * rect.height / 2f, btnSize, btnSize);
                        switch (btnIdx)
                        {
                            case 0:
                                if (GUI.Button(btnRect, "R")) uvOffset = Vector2.zero;
                                break;
                            case 1:
                                if (GUI.Button(btnRect, "▲")) uvOffset.y -= m_brush.Tileset.AtlasTexture.texelSize.y;
                                break;
                            case 3:
                                if (GUI.Button(btnRect, "▶")) uvOffset.x -= m_brush.Tileset.AtlasTexture.texelSize.x;
                                break;
                            case 4:
                                if (GUI.Button(btnRect, "▼")) uvOffset.y += m_brush.Tileset.AtlasTexture.texelSize.y;
                                break;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (tileSelection != null)
            {
                ...
                {
                    ((AnimBrush) target).AnimFrames.AddRange(
                        tileSelection.selectionData.Select(x => new AnimBrush.TileAnimFrame() {tileId = x}));
                }
            }
            ...
            EditorGUILayout.Space();
            uint brushTileData = m_frameList.index >= 0
                ? m_brush.AnimFrames[m_frameList.index].tileId
            ...
            EditorGUILayout.HelpBox("Select a frame from list and then select a tile from tile selection window.",
                MessageType.Info);
