...
namespace CreativeSpore.SuperTilemapEditor
{
    public class TileGridControl
    {
        ...
        public TileGridControl(Object target, int width, int height, Texture2D backgroundTexture,
            GetTileData getTileDataFunc, SetTileData setTileDataFunc)
        {
            m_target = target;
            m_width = width;
            m_height = height;
        ...
        public void Display(Vector2 visualTileSize)
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                Rect rArea = GUILayoutUtility.GetLastRect();
                {
                    if (m_backgroundTexture)
                        GUI.DrawTexture(
                            new Rect(rArea.position, Vector2.Scale(visualTileSize, new Vector2(m_width, m_height))),
                            m_backgroundTexture);
                    ...
                    for (int tileIdx = 0; tileIdx < size; ++tileIdx)
                    {
                        ...
                        HandlesEx.DrawRectWithOutline(rVisualTile,
                            ...
                            m_selectedTileIdx == tileIdx ? cSelectedBorderColor : bgColor);
                ...
                uint brushTileData = m_selectedTileIdx >= 0
                    ? m_getTileDataFunc(m_selectedTileIdx)
            ...
            if (Tileset.TileSelection != null && Tileset.TileSelection.selectionData.Count == m_width * m_height &&
                Tileset.TileSelection.rowLength == m_width)
            {
