...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class BrushBehaviourEditor : Editor
    {
        ...
        private static GameObject CreateTilemapFromBrush()
        {
            ...
            {
                GameObject brushTilemap =
                    new GameObject(GameObjectUtility.GetUniqueNameForSibling(null, "TilemapSelection"));
        ...
        private static void CreatePrefabFromBrush()
        {
            if (BrushBehaviour.Exists)
            {
                ...
                path = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(path), brushTilemap.name + ".prefab")
                    .Replace(@"\", @"/");
