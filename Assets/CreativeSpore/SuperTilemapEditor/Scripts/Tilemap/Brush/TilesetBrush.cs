...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class TilesetBrush : ScriptableObject, IBrush
    {
        ...
        {
            get { return m_autotilingMode; }
        }

        public bool ShowInPalette
        {
            get { return m_showInPalette; }
            set { m_showInPalette = value; }
        }

        [SerializeField] private int m_group = 0;
        [SerializeField] private eAutotilingMode m_autotilingMode = eAutotilingMode.Self;
        [SerializeField] private bool m_showInPalette = true;
        ...
        public bool AutotileWith(int selfBrushId, int otherBrushId)
        {
            if (
                ...
                ((AutotilingMode & eAutotilingMode.Other) != 0 && otherBrushId != selfBrushId &&
                 otherBrushId != (Tileset.k_TileDataMask_BrushId >> 16))
            )
            {
                return true;
            }
            ...
            if ((AutotilingMode & eAutotilingMode.Group) != 0)
            {
                ...
                if (brush)
                    ...
                    return
                        Tileset.GetGroupAutotiling(Group,
                            0); //with normal tiles, use default group (0) //TODO: this is old code, now it is possible to change tile group. Check if this method is useful after removing it from TilemapChunk
            }
        ...
        {
            bool isOutOfBounds = gridX > tilemap.MaxGridX || gridX < tilemap.MinGridX || gridY > tilemap.MaxGridY ||
                                 gridY < tilemap.MinGridY;
        ...
        {
            if ((AutotilingMode & eAutotilingMode.EmptyCells) != 0 &&
                otherTileData == Tileset.k_TileData_Empty) return true;
            if ((AutotilingMode & eAutotilingMode.Group) != 0)
            {
        ...
        public virtual Vector2[] GetMergedSubtileColliderVertices(STETilemap tilemap, int gridX, int gridY,
            uint tileData)
        {
            return null;
        }
