...
namespace CreativeSpore.SuperTilemapEditor
{
    partial class TilemapChunk
    {
        ...
        public bool UpdateColliders()
        {
            ...
            if (m_needsRebuildColliders)
            {
                if (m_has2DColliders)
                {
                    ...
                    System.Type oppositeCollider2DType = ParentTilemap.Collider2DType != e2DColliderType.EdgeCollider2D
                        ? typeof(EdgeCollider2D)
                        : typeof(PolygonCollider2D);
                    System.Type collidersToDestroy = ParentTilemap.ColliderType != eColliderType._2D
                        ? typeof(Collider2D)
                        : oppositeCollider2DType;
            ...
            if (m_needsRebuildColliders)
            {
                ...
                if (ParentTilemap.ColliderType == eColliderType._3D)
                {
                    ...
                    m_meshCollider.sharedMesh =
                        null; // for some reason this fix showing the green lines of the collider mesh
        ...
        private bool FillColliderMeshData()
        {
            ...
            System.Type collider2DType = ParentTilemap.Collider2DType == e2DColliderType.EdgeCollider2D
                ? typeof(EdgeCollider2D)
                : typeof(PolygonCollider2D);
            ...
            for (int ty = 0, tileIdx = 0; ty < m_height; ++ty)
            {
                for (int tx = 0; tx < m_width; ++tx, ++tileIdx)
                {
                    ...
                    if (tileData != Tileset.k_TileData_Empty)
                    {
                        ...
                        Vector2[] subTileMergedColliderVertices =
 brush ? brush.GetMergedSubtileColliderVertices(ParentTilemap, GridPosX + tx, GridPosY + ty, tileData) : null;
#else
                        ...
                        if (tile != null)
                        {
                            ...
                            {
                                isEmpty = false;
                                int neighborCollFlags =
                                    0; // don't remove, even using neighborTileCollData, neighborTileCollData is not filled if tile is empty
                                ...
                                {
                                    if ((tileData & (Tileset.k_TileFlag_FlipH | Tileset.k_TileFlag_FlipV |
                                                     Tileset.k_TileFlag_Rot90)) != 0)
                                    {
                                    ...
                                    for (int i = 0; i < 4; ++i)
                                    {
                                        ...
                                        bool isTriggerOrPolygon = ParentTilemap.IsTrigger ||
                                                                  ...
                                                                  ParentTilemap.Collider2DType ==
                                                                  e2DColliderType.PolygonCollider2D;
                                        switch (i)
                                        {
                                            case 0: // Up Tile
                                                neighborTileData = (tileIdx + m_width) < m_tileDataList.Count
                                                    ...
                                                    : isTriggerOrPolygon
                                                        ? Tileset.k_TileData_Empty
                                                        : ParentTilemap.GetTileData(GridPosX + tx, GridPosY + ty + 1);
                                                break;
                                            case 1: // Right Tile
                                                neighborTileData = (tileIdx + 1) % m_width != 0
                                                    ? //(tileIdx + 1) < m_tileDataList.Count ? 
                                                    m_tileDataList[tileIdx + 1]
                                                    : isTriggerOrPolygon
                                                        ? Tileset.k_TileData_Empty
                                                        : ParentTilemap.GetTileData(GridPosX + tx + 1, GridPosY + ty);
                                                break;
                                            case 2: // Down Tile
                                                ...
                                                break;
                                            case 3: // Left Tile
                                                neighborTileData = tileIdx % m_width != 0
                                                    ? //neighborTileId = tileIdx >= 1 ? 
                                                    m_tileDataList[tileIdx - 1]
                                                    : isTriggerOrPolygon
                                                        ? Tileset.k_TileData_Empty
                                                        : ParentTilemap.GetTileData(GridPosX + tx - 1, GridPosY + ty);
                                                break;
                                            default:
                                                neighborTileData = Tileset.k_TileData_Empty;
                                                break;
                                        }
                                        ...
                                        if (neighborTileId != Tileset.k_TileId_Empty &&
                                            neighborTileId >= Tileset.Tiles.Count)
                                            Debug.LogWarning("Wrong neighbor tileId " + neighborTileId +
                                            ...
                                            neighborTileId < Tileset.Tiles.Count)
                                        {
                                            ...
                                            if ((neighborTileData &
                                                 (Tileset.k_TileFlag_FlipH | Tileset.k_TileFlag_FlipV |
                                                  Tileset.k_TileFlag_Rot90)) != 0)
                                            {
                                            ...
                                            isSurroundedByFullColliders &=
                                                (neighborTileCollider.type == eTileCollider.Full);

                                            if (neighborTileCollider.type == eTileCollider.None)
                                            {
                                                segmentMinMax =
                                                    new Vector2(float.MaxValue,
                                                        float.MinValue); //NOTE: x will be min, y will be max
                                            }
                                            else if (neighborTileCollider.type == eTileCollider.Full)
                                            {
                                                segmentMinMax =
                                                    new Vector2(0f, 1f); //NOTE: x will be min, y will be max
                                ...
                                else
                                {
                                    ...
                                    if (!hasMergedColliders)
                                        collVertices = tileCollData.type == eTileCollider.Full
                                    ...
                                    for (int i = 0; i < collVertices.Length; ++i)
                                    {
                                        ...
                                        if ((tileCollData.type == eTileCollider.Full) &&
                                            (
                                                (i == 0 && neighborTileCollData[3].type ==
                                                 eTileCollider.Full) || // left tile has collider
                                                (i == 1 && neighborTileCollData[0].type ==
                                                 eTileCollider.Full) || // top tile has collider
                                                (i == 2 && neighborTileCollData[1].type ==
                                                 eTileCollider.Full) || // right tile has collider
                                                (i == 3 && neighborTileCollData[2].type ==
                                                 eTileCollider.Full) // bottom tile has collider
                                            )
                                        )
                                        {
                                            continue;
                                        ...
                                        else // if( tileCollData.type == eTileCollider.Polygon ) Or Full colliders if neighbor is not Full as well
                                        {
                                            ...
                                            else if (s0.y == 1f && s1.x == 1f) // top - right diagonal
                                            {
                                                if ((neighborCollFlags & 0x1) != 0 && (neighborCollFlags & 0x2) != 0)
                                                {
                                                    ...
                                                    if ((n.x < n.y && n.x <= s0.x && n.y == 1f) &&
                                                        (m.x < m.y && m.x <= s1.y && m.y == 1f))
                                                    {
                                                        continue;
                                                    }
                                                }
                                            ...
                                            else if (s0.x == 1f && s1.y == 0f) // right - bottom diagonal
                                            {
                                                if ((neighborCollFlags & 0x2) != 0 && (neighborCollFlags & 0x4) != 0)
                                                {
                                                    ...
                                                    if ((n.x < n.y && n.x == 0f && n.y >= s0.y) &&
                                                        (m.x < m.y && m.x <= s1.x && m.y == 1f))
                                                    {
                                                        continue;
                                                    }
                                                }
                                            ...
                                            else if (s0.y == 0f && s1.x == 0f) // bottom - left diagonal
                                            {
                                                if ((neighborCollFlags & 0x4) != 0 && (neighborCollFlags & 0x8) != 0)
                                                {
                                                    ...
                                                    if ((n.x < n.y && n.x == 0f && n.y >= s0.x) &&
                                                        (m.x < m.y && m.x == 0f && m.y >= s1.y))
                                                    {
                                                        continue;
                                                    }
                                                }
                                            ...
                                            else if (s0.x == 0f && s1.y == 1f) // left - top diagonal
                                            {
                                                if ((neighborCollFlags & 0x8) != 0 && (neighborCollFlags & 0x1) != 0)
                                                {
                                                    ...
                                                    if ((n.x < n.y && n.x <= s0.y && n.y == 1f) &&
                                                        (m.x < m.y && m.x == 0f && m.y >= s1.x))
                                                    {
                                                        continue;
                                                    }
                                                }
                                        ...
                                        s0.x = px0 + CellSize.x * s0.x;
                                        s0.y = py0 + CellSize.y * s0.y;
                                        s1.x = px0 + CellSize.x * s1.x;
                                        s1.y = py0 + CellSize.y * s1.y;
                                        if (ParentTilemap.ColliderType == eColliderType._3D)
                                        {
                                        ...
                                        else //if( ParentTilemap.ColliderType == eColliderType._2D )
                                        {
                                            ...
                                            for (int edgeIdx = s_openEdges.Count - 1;
                                                edgeIdx >= 0 && linkedSegments < 2;
                                                --edgeIdx)
                                            {
                                                ...
                                                if (edgeSegments.Last.Value == s0)
                                                {
                                                    ...
                                                    {
                                                        LinkedList<Vector2> segmentToMerge =
                                                            s_openEdges[segmentIdxToMerge];
                                                        if (s0 == segmentToMerge.First.Value)
                                                        {
                                                            for (LinkedListNode<Vector2> node =
                                                                    segmentToMerge.First.Next;
                                                                node != null;
                                                                node = node.Next)
                                                ...
                                                else if (edgeSegments.First.Value == s1)
                                                {
                                                    if (segmentIdxToMerge >= 0)
                                                    {
                                                        ...
                                                        {
                                                            for (LinkedListNode<Vector2> node = edgeSegments.First.Next;
                                                                node != null;
                                                                node = node.Next)
            ...
            if (ParentTilemap.ColliderType == eColliderType._2D)
            {
                ...
                for (int i = 0; i < s_openEdges.Count; ++i)
                {
                    ...
                    Collider2D collider2D = reuseCollider
                        ? (Collider2D) aColliders2D[i]
                        : (Collider2D) gameObject.AddComponent(collider2DType);
