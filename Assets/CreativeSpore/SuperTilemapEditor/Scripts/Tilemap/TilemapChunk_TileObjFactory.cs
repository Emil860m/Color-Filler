    ...
    partial class TilemapChunk
    {
        ...
        public struct OnTilePrefabCreationData
        {
            ...
            public ParameterContainer Parameters
            {
                get
                {
                    return TilemapUtils.GetParamsFromTileData(ParentTilemap, ParentTilemap.GetTileData(GridX, GridY));
                }
            }

            public Tile Tile
            {
                get { return ParentTilemap.GetTile(GridX, GridY); }
            }

            public TilesetBrush Brush
            {
                get { return ParentTilemap.GetBrush(GridX, GridY); }
            }
        }
        ...
        private GameObject CreateTileObject(int tileIdx, TilePrefabData tilePrefabData)
        {
            if (tilePrefabData.prefab != null)
            {
                ...
                if (tileObjData == null || tileObjData.tilePrefabData != tilePrefabData || tileObjData.obj == null)
                {
                    ...
                    {
                        UnityEditor.Undo.RegisterCreatedObjectUndo(tileObj,
                            STETilemap.k_UndoOpName + ParentTilemap.name);
                    }
#else
...
//+++ Break tilemap prefab and restore tile prefab link
 ...
 UnityEditor.PrefabUtility.FindRootGameObjectWithSameParentPrefab(tileObjData.obj);
                    if (parentPrefab != tileObjData.obj)
                    {
                        DestroyImmediate(tileObjData.obj);
                        tileObjData.obj =
 UnityEditor.PrefabUtility.InstantiatePrefab(tileObjData.tilePrefabData.prefab) as GameObject;
                    }
                    ///---
#endif
            ...
            uint tileData)
        {
            Vector3 chunkLocPos = new Vector3((gx + .5f) * CellSize.x, (gy + .5f) * CellSize.y,
                tilePrefabData.prefab.transform.position.z);
            ...
            tileObj.transform.localRotation = tilePrefabData.prefab.transform.localRotation *
                                              Quaternion.Euler(tilePrefabData.rotation);
